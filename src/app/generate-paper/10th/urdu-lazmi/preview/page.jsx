...
// ... existing code ...
  ...
  useEffect(() => {
    ...
    const savedExercises = localStorage.getItem("selectedUrduLazmi10Exercises");
    if (savedExercises) {
      try {
  ...
  const getMarksPerQuestion = (type) => {
    switch (type) {
      ...
      case "essay":
        return 15; // Essay - 15 marks
      case "comprehense":
        return 10; // Comprehension - 10 marks
      case "cen_idea":
      case "story":
      case "dialogue":
      case "sen_corr":
...
const preserveLineBreaks = (text, type) => {
  ...
  switch (type) {
    ...
    case "essay":
      ...
      break;
    case "comprehense":
      lineHeight = paperSettings.comprehenseLineHeight;
      marginBottom = "0.3 rem";
      break;
    case "cen_idea":
...
// ... existing code ...
    ...
    setSections((prevSections) => {
      ...
      Object.entries(questionsByType).forEach(([type, questions]) => {
        ...
        switch (type) {
          ...
          case "provrb_exp":
            choiceValue = Math.min(
              paperSettings.provrb_expChoice || 0,
              questions.length - 1
            );
            break;
          case "short":
          ...
          case "comprehense":
            choiceValue = Math.min(
              paperSettings.comprehenseChoice || 0,
              questions.length - 1
            );
            break;
          default:
        ...
        switch (type) {
          ...
          case "poet_ex":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} نظم و غزل کے اشعار کی تشریح کیجیے`
              : "مندرجہ ذیل نظم و غزل کے اشعار کی تشریح کیجیے";
            break;
          case "provrb_exp":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} نثر پاروں کی تشریح کیجیے۔سبق کاعنوان، مصنف کا نام اور خط کشیدہ الفاظ کے معنی بھی تحریر کیجیے۔`
              : "درج ذیل نثر پاروں کی تشریح کیجیے۔سبق کاعنوان، مصنف کا نام اور خط کشیدہ الفاظ کے معنی بھی تحریر کیجیے۔";
            break;
          case "short":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} مختصر سوالات کے جوابات دیں۔`
              : "درج ذیل سوالوں کے مختصر جوابات دیں۔";
            break;
          case "summary":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} سبق کا خلاصہ لکھیے۔`
              : "درج ذیل سبق کا خلاصہ لکھیے۔";
            break;
          case "essay":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} عنوان پر مضمون لکھیں۔`
              : "درج ذیل عنوان پر مضمون لکھیں۔";
            break;
          case "comprehense":
            title = "درج ذیل عبارت کو غور سے پڑھیے اور آخر میں دیے گئے سوالات کے جوابات لکھیے۔";
            break;
          default:
  ...
  const handlePrintClick = () => {
    ...
    const filename = `10th-Urdu-${paperSettings.section || "section"}-${
...
`);
  ...
  const getHeadingTextByType = (type, section) => {
    ...
    switch (type) {
      ...
      case "summary":
        ...
        break;
      case "essay":
        choiceValue = paperSettings.essayChoice || 0;
        break;
      case "comprehense":
        choiceValue = paperSettings.comprehenseChoice || 0;
        break;
      case "cen_idea":
    ...
    switch (type) {
      ...
      case "essay":
        return `مضمون (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      case "comprehense":
        return `تفہیم عبارت (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      default:
        return "";
    }
  };
  ...
  const calculateMarksForQuestionType = (questions, type) => {
    ...
    if (["mcqs", "poet_ex", "provrb_exp", "short", "summary", "essay", "comprehense"].includes(type)) {
  ...
  const calculatePaperTotalMarks = () => {
    ...
    const questionTypes = [
      "mcqs", "short", "poet_ex", "provrb_exp", "summary", 
      "essay", "comprehense", "cen_idea", "letter", "application", 
      "story", "dialogue", "sen_corr"
    ];
  ...
  <div className="grid grid-cols-3 gap-6">
    <div className="flex items-center gap-3">
      ...
      >
        ...
        <option className="font-nastaliq" value="provrb_exp">نثر پاروں کی تشریح</option>
    ...
    <div className="grid grid-cols-2 gap-4 mt-2">
      <div className="flex items-center gap-2">
        ...
        <input
          ...
          onChange={(e) => setPaperSettings(prev => ({...prev, essayChoice: Number.parseInt(e.target.value) || 0}))}
          className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
        />
      </div>
      <div className="flex items-center gap-2">
        ...
        <input
          type="number"
          min="0"
          value={paperSettings.comprehenseChoice || 0}
          onChange={(e) => setPaperSettings(prev => ({...prev, comprehenseChoice: Number.parseInt(e.target.value) || 0}))}
          className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
        />
      </div>
    </div>
...
<div className="mt-6 space-y-4 border-t border-gray-200 pt-4">
  ...
  <div className="grid grid-cols-2 gap-4">
    ...
    <div>
      <label className="block text-xs text-gray-600 mb-1">
        Comprehension Line Height
      </label>
      <input
        type="number"
        step="0.1"
        value={paperSettings.comprehenseLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            comprehenseLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
