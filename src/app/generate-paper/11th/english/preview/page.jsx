...
import { nastaliq } from "../../../../fonts" // Add this import

// Add this function after your imports
const calculateRequiredQuestions = (questions, choiceCount, type) => {
  const typeQuestions = questions.filter((q) => q.type === type)
  const sectionConfig = sections[type]?.[0]
  const requiredCount = sectionConfig?.choice || choiceCount

  // Ensure we don't return a negative value
  return requiredCount > 0 ? Math.max(0, typeQuestions.length - requiredCount) : typeQuestions.length
}

const isUrduText = (text) => /[\u0600-\u06FF]/.test(text)

// Remove this import as we'll fetch from the database instead
// import { ninthEnglish} from "@/data/dummyQuestions"
...
export default function PaperPreview() {
  ...
  sq_book_i: [
    {
      title: "Write short answers to the following questions (From Book I)",
      questions: [],
      marks: 2,
      choice: 0,
    },
  ...
  sqBoPlays: [
    {
      title: "Write short answers to the following questions (From Book III Plays)",
      questions: [],
      marks: 2,
      choice: 0,
    },
  ...
  sqBokPoems: [
    {
      title: "Write short answers to the following questions (From Book III Poems)",
      questions: [],
      marks: 2,
      choice: 0,
    },
  ...
  story: [
    {
      title: "Write a story being a moral lesson",
      questions: [],
      marks: 10,
      choice: 0,
    },
  ...
  exp_poems: [
    {
      title: "Translate the following lines with reference to the context.",
      questions: [],
      marks: 5,
      choice: 0,
    },
  ...
  punctuation: [
    {
      title: "Punctuate the following extract",
      questions: [],
      marks: 5,
      choice: 0,
    },
  ...
  pair_o_w: [
    {
      title: "Use following pair of words in your own sentences.",
      questions: [],
      marks: 1,
      choice: 0,
    },
  ...
  e_to_u: [
    {
      title: "Translate the following passage into urdu.",
      questions: [],
      marks: 15,
      choice: 0,
    },
  ...
  const [paperSettings, setPaperSettings] = useState({
    ...
    essayLineHeight: 1.2,
    storyLineHeight: 1.2,
    dialogueLineHeight: 1.2,
    applicationLineHeight: 1.2,
    summaryLineHeight: 1.2,
    pair_o_wLineHeight: 1.2,
  ...
  // Update the `getUrduTextStyle` function to ensure it's properly applied:

  const getUrduTextStyle = () => ({
    fontFamily: `${nastaliq.style.fontFamily}, sans-serif`,
    fontSize: `${paperSettings.textFontSize}px`,
    color: paperSettings.textFontColor || "black",
    lineHeight: paperSettings.lineHeight || "1.5",
     direction: 'rtl'
  })
  ...
  const handleMcqsChoiceChange = (value) => {
  setPaperSettings((prev) => ({
    ...prev,
    mcqsChoice: Math.max(0, Number.parseInt(value) || 0),
  }))
}

const handleEssayChoiceChange = (value) => {
  setPaperSettings((prev) => ({
    ...prev,
    essayChoice: Math.max(0, Number.parseInt(value) || 0),
  }))
}

const handleSummaryChoiceChange = (value) => {
  setPaperSettings((prev) => ({
    ...prev,
    summaryChoice: Math.max(0, Number.parseInt(value) || 0),
  }))
}

const handleEToUChoiceChange = (value) => {
  setPaperSettings((prev) => ({
    ...prev,
    e_to_uChoice: Math.max(0, Number.parseInt(value) || 0),
  }))
}

const handleUToEChoiceChange = (value) => {
  setPaperSettings((prev) => ({
    ...prev,
    u_to_eChoice: Math.max(0, Number.parseInt(value) || 0),
  }))
}

const handleDirIndChoiceChange = (value) => {
  setPaperSettings((prev) => ({
    ...prev,
    dir_indChoice: Math.max(0, Number.parseInt(value) || 0),
  }))
}

const handlePairOWChoiceChange = (value) => {
  setPaperSettings((prev) => ({
    ...prev,
    pair_o_wChoice: Math.max(0, Number.parseInt(value) || 0),
  }))
}

const handleSqBookIChoiceChange = (value) => {
  setPaperSettings((prev) => ({
    ...prev,
    sq_book_iChoice: Math.max(0, Number.parseInt(value) || 0),
  }))
}

const handleSqBoPlaysChoiceChange = (value) => {
  setPaperSettings((prev) => ({
    ...prev,
    sqBoPlaysChoice: Math.max(0, Number.parseInt(value) || 0),
  }))
}

const handleSqBokPoemsChoiceChange = (value) => {
  setPaperSettings((prev) => ({
    ...prev,
    sqBokPoemsChoice: Math.max(0, Number.parseInt(value) || 0),
  }))
}

const handleLetterChoiceChange = (value) => {
  setPaperSettings((prev) => ({
    ...prev,
    letterChoice: Math.max(0, Number.parseInt(value) || 0),
  }))
}

const handleApplicationChoiceChange = (value) => {
  setPaperSettings((prev) => ({
    ...prev,
    applicationChoice: Math.max(0, Number.parseInt(value) || 0),
  }))
}

const handleStoryChoiceChange = (value) => {
  setPaperSettings((prev) => ({
    ...prev,
    storyChoice: Math.max(0, Number.parseInt(value) || 0),
  }))
}

const handleExpPoemsChoiceChange = (value) => {
  setPaperSettings((prev) => ({
    ...prev,
    exp_poemsChoice: Math.max(0, Number.parseInt(value) || 0),
  }))
}

const handlePunctuationChoiceChange = (value) => {
  setPaperSettings((prev) => ({
    ...prev,
    punctuationChoice: Math.max(0, Number.parseInt(value) || 0),
  }))
}
  ...
  const filterQuestions = () => {
    ...
    const filtered = allQuestions.filter((question) => {
        ...
        (question.type &&
          ...
          (question.type.toLowerCase() === questionType.toLowerCase() ||
            ...
            (questionType.toLowerCase() === "long" &&
              (question.type.toLowerCase() === "long" || question.type.toLowerCase().includes("long")))))
      ...
      if (!matchesType && questionType) {
        console.log(`Question ${question.id} type mismatch: ${question.type} vs ${questionType}`)
      }

      if (!matchesExercises && selectedExercises.length) {
        console.log(`Question ${question.id} exercise mismatch: ${question.topic || question.exercise}`)
      }
  ...
  useEffect(() => {
    ...
    const savedExercises = localStorage.getItem("selectedEnglishExercises")
    if (savedExercises) {
      try {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const subject = paperSettings.subject || "English" // Get current subject

      // First try to load subject-specific settings
      const savedSubjectSettings = localStorage.getItem(`paperSettings_${deviceId}_${subject}`)
      if (savedSubjectSettings) {
        try {
          ...
          console.log(`Loaded saved settings for subject ${subject} on device:`, deviceId)
          return
        } catch (error) {
          console.error(`Error parsing saved subject settings for ${subject}:`, error)
        }
      }

      // If no subject-specific settings exist, try to load device-wide settings
      const savedSettings = localStorage.getItem(`paperDefaultSettings_${deviceId}`)
      if (savedSettings) {
        try {
          ...
          localStorage.setItem(`paperSettings_${deviceId}_${subject}`, savedSettings)
          console.log(`Created new settings for subject ${subject} based on device settings`)
        } catch (error) {
        ...
        localStorage.setItem(`paperSettings_${deviceId}_${subject}`, JSON.stringify(paperSettings))
...
// Replace the getMarksPerQuestion function with this updated version
  ...
  // Find the totalMarks useMemo calculation and replace it with this improved version
  // Replace the totalMarks useMemo calculation with this corrected version
// ... existing code ...
    ...
    if (typeSections && typeSections.length > 0) {
      typeSections.forEach((section) => {
        if (section && section.questions && section.questions.length > 0) {
          ...
          // Calculate how many questions student needs to attempt
          const questionsToAttempt = Math.max(0, questionsCount - choiceValue)
          
          // Calculate marks based on questions to attempt
          marks += questionsToAttempt * marksPerQuestion
        }
      })
    }
  })
...
// ... existing code ...
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const params = new URLSearchParams({
          class: "11th",
          subject: "English", // Always use English for the initial fetch
        })
        ...
        console.log("Total questions fetched:", data.length || (data.questions && data.questions.length))
        ...
        const processedQuestions = filteredQuestions.map((question) => {
          ...
          if (question.type && (question.type === "long" || question.type.includes("long"))) {
            return {
              ...question,
  ...
  useEffect(() => {
    ...
    console.log("Current state - searchedQuestions count:", searchedQuestions.length)
  }, [questionType, selectedExercises, criteria, allQuestions.length, searchedQuestions.length])
  ...
  const getTextStyle = (type = "normal", text = "") => {
    // Check if text contains Urdu characters
    const isUrdu = isUrduText(text)

    // Apply Urdu font if detected
    if (isUrdu) {
      return {
        fontFamily: nastaliq.style.fontFamily,
        fontSize: `${paperSettings.textFontSize}px`,
...
const getQuestionTextStyle = (type, text) => {
  const isUrduText = /[\u0600-\u06FF]/.test(text);
  
  if (type === "u_to_e" || isUrduText) {
    return {
      fontFamily: `${nastaliq.style.fontFamily}, sans-serif`,
      fontSize: `${paperSettings.textFontSize}px`,
      direction: 'rtl',
      lineHeight: paperSettings.u_to_eLineHeight || '2.5',
      textAlign: 'right'
    };
  }
  
  // Base style for all other questions
  const baseStyle = {
    fontSize: `${paperSettings.textFontSize}px`,
  ...
  switch (type) {
    ...
    case "long":
      ...
      break;
    case "e_to_u":
      lineHeight = paperSettings.e_to_uLineHeight || paperSettings.lineHeight;
      break;
    case "u_to_e":
      lineHeight = paperSettings.u_to_eLineHeight || paperSettings.lineHeight;
      break;
    case "summary":
      lineHeight = paperSettings.summaryLineHeight || paperSettings.lineHeight;
      break;
    case "essay":
      lineHeight = paperSettings.essayLineHeight;
      break;
    case "dir_ind":
      lineHeight = paperSettings.dir_indLineHeight || paperSettings.lineHeight;
      break;
    case "pair_o_w":
      lineHeight = paperSettings.pair_o_wLineHeight || paperSettings.lineHeight;
      break;
    default:
      lineHeight = paperSettings.lineHeight;
  }
...
// ... existing code ... 
  ...
  const handleRandomSelect = () => {
    ...
    const availableQuestions = searchedQuestions.filter((q) => !usedQuestionIds.has(q.id))
    ...
    const count = Math.min(Number.parseInt(requiredCount), availableQuestions.length)
  ...
  // Modify the handleAddSelection function to ensure Urdu text detection is preserved
const handleAddSelection = () => {
  const selectedQuestionsData = searchedQuestions.filter((q) => selectedQuestionIds.has(q.id))

  // Process each selected question to ensure Urdu text is properly handled
  const newSelectedQuestions = searchedQuestions
    .filter((question) => selectedQuestionIds.has(question.id))
    .map((question) => {
      // Check if the question text contains Urdu
      const hasUrduText = isUrduText(question.text)
      
      return {
        ...question,
        images: question.images || [], // Ensure images are included in the transfer
        hasUrduText: hasUrduText, // Flag questions with Urdu text
      }
    })
  ...
  // Process each selected question to ensure images and Urdu text flags are properly included
  const processedQuestions = selectedQuestionsData.map((question) => {
    const hasUrduText = isUrduText(question.text)
    
    // Also check options for MCQs if they contain Urdu
    let hasUrduOptions = false
    if (question.type === "mcqs" && question.options && Array.isArray(question.options)) {
      hasUrduOptions = question.options.some(option => isUrduText(option))
    }
    
    // Check parts for long questions if they contain Urdu
    let hasUrduParts = false
    if (question.parts && Array.isArray(question.parts)) {
      hasUrduParts = question.parts.some(part => isUrduText(part))
    }
    ...
    return {
      ...
      hasUrduText: hasUrduText, // Flag questions with Urdu text
      hasUrduOptions: hasUrduOptions, // Flag MCQs with Urdu options
      hasUrduParts: hasUrduParts, // Flag long questions with Urdu parts
    }
  })
  ...
  setSections((prevSections) => {
    ...
    Object.entries(questionsByType).forEach(([type, questions]) => {
      ...
      const choiceValue = (() => {
        switch (type) {
          case "mcqs":
            return Math.min(paperSettings.mcqsChoice || 0, questions.length - 1)
          case "sq_book_i":
            return Math.min(paperSettings.sq_book_iChoice || 0, questions.length - 1)
          case "sqBoPlays":
            return Math.min(paperSettings.sqBoPlaysChoice || 0, questions.length - 1)
          case "sqBokPoems":
            return Math.min(paperSettings.sqBokPoemsChoice || 0, questions.length - 1)
          case "letter":
            return Math.min(paperSettings.letterChoice || 0, questions.length - 1)
          case "application":
            return Math.min(paperSettings.applicationChoice || 0, questions.length - 1)
          case "story":
            return Math.min(paperSettings.storyChoice || 0, questions.length - 1)
          case "exp_poems":
            return Math.min(paperSettings.exp_poemsChoice || 0, questions.length - 1)
          case "punctuation":
            return Math.min(paperSettings.punctuationChoice || 0, questions.length - 1)
          case "pair_o_w":
            return Math.min(paperSettings.pair_o_wChoice || 0, questions.length - 1)
          case "e_to_u":
            return Math.min(paperSettings.e_to_uChoice || 0, questions.length - 1)
          default:
            return 0
        }
      })()
      ...
      const title = getHeadingTextByType(type, { questions, choice: choiceValue })
...
const handleSearchQuestions = () => {
  ...
  // Build query parameters - IMPORTANT: Always use English as the subject since that's what the topics are for
  const params = new URLSearchParams({
    class: "11th",
    subject: "English", // Hardcode to English since the topics are from English
  })
  ...
  fetch(`${apiUrl}?${params.toString()}`)
    ...
    .then((data) => {
      ...
      if (questions.length === 0) {
        setNotification({
          show: true,
          message: "No questions found with your criteria. Please check your filters or try different topics.",
          type: "warning",
        })
        setLoading(false)
        return
      ...
      const topicFilteredQuestions = questions.filter((question) => {
        ...
        if (!topicMatch) {
          console.log(`Question ${question.id} topic mismatch: ${question.topic} not in selected topics`)
        }
        return topicMatch
      })

      console.log(`After topic filtering: ${topicFilteredQuestions.length} questions remaining`)
          ...
          if (question.parts) {
            if (!Array.isArray(question.parts)) {
              try {
                ...
                question.parts = typeof question.parts === "string" ? JSON.parse(question.parts) : []
              } catch (e) {
      ...
      console.log("Initial questions count after processing:", filteredQuestions.length)
      ...
      if (medium && medium !== "") {
        ...
        filteredQuestions = filteredQuestions.filter((question) => {
          ...
          if (!matches) {
            console.log(`Question ${question.id} medium mismatch: ${question.medium} vs ${medium}`)
          }
          return matches
        })
        console.log(`After medium filter: ${filteredQuestions.length} questions remaining`)
      }
      ...
      if (selectionSource && selectionSource !== "") {
        ...
        filteredQuestions = filteredQuestions.filter((question) => {
          const matches = question.source === source
          if (!matches) {
            console.log(`Question ${question.id} source mismatch: ${question.source} vs ${selectionSource}`)
          }
          return matches
        })
        console.log(`After source filter: ${filteredQuestions.length} questions remaining`)
      }
      ...
      if (filteredQuestions.length > 0) {
        ...
        setNotification({
          show: true,
          message: "No questions match your criteria after filtering. Try adjusting your filters.",
        })
      }
      setLoading(false)
    })
...
const handlePrintClick = () => {
  ...
  const filename = `11th-English-${paperSettings.section || "section"}-${
...
const handleSavePaper = async () => {
  try {
    ...
    localStorage.setItem(`paperSettings_${deviceId}_${subject}`, JSON.stringify(paperSettings))
    console.log(`Saved current settings for subject ${subject} to localStorage`)
    ...
    const paperData = {
      ...
      subject: paperSettings.subject || "English",
...
const groupQuestionsByType = (questions) => {
  // Group questions by their type based on all available section types
...
// Update the getHeadingTextByType function to better handle different question types with more specific headings
const getHeadingTextByType = (type, section) => {
  // Use the section's title if it exists, otherwise use default
  if (
    section.title &&
    section.title !== `ATTEMPT ANY ${section.required || 1} OF THE FOLLOWING ${section.questions.length} QUESTIONS`
  ) {
  ...
  // If there's a choice value and enough questions, show the "ATTEMPT ANY X OUT OF Y" message
  ...
  // Otherwise, show the default heading for the question type
  switch (type) {
    case "mcqs":
      return "Choose the correct option."
    case "sq_book_i":
      return "Write short answers to the following questions (From Book I)."
    case "sqBoPlays":
      return "Write short answers to the following questions (From Book III Plays)."
    case "sqBokPoems":
      return "Write short answers to the following questions (From Book III Poems)."
    case "letter":
      return "Write a letter to."
    case "application":
      return "Write an application to."
    case "story":
      return "Write a story being a moral lesson."
    case "exp_poems":
      return "Translate the following lines with reference to the context."
    case "punctuation":
      return "Punctuate the following extract."
    case "pair_o_w":
      return "Use following pair of words in your own sentences."
    case "e_to_u":
      return "Translate the following passage into urdu."
    default:
      return ""
  }
}

// Update the calculateMarksForQuestionType function to prevent negative values
// Replace the calculateMarksForQuestionType function with this corrected version
  const calculateMarksForQuestionType = (questions, type, sectionChoice = 0) => {
    const totalQuestions = questions.length
    const marksPerQuestion = getMarksPerQuestion(type)

    // Apply choice to calculate how many questions need to be attempted
    const questionsToAttempt = Math.max(0, totalQuestions - sectionChoice)
    
    // Calculate total marks based on questions to attempt
    return questionsToAttempt * marksPerQuestion
  }

// Also update the calculateTotalMarks function to ensure it never returns negative values
const calculateTotalMarks = () => {
  const mcqsMarks = selectedQuestions.filter((q) => q.type === "mcqs").reduce((sum, q) => sum + (q.marks || 0), 0)

  const shortQuestions = selectedQuestions.filter((q) => q.type === "short")
  const shortRequired = Math.max(0, calculateRequiredQuestions(selectedQuestions, paperSettings.shortChoice, "short"))
  const shortMarks = shortQuestions.slice(0, shortRequired).reduce((sum, q) => sum + (q.marks || 0), 0)

  const longQuestions = selectedQuestions.filter((q) => q.type === "long")
  const longRequired = Math.max(0, calculateRequiredQuestions(selectedQuestions, paperSettings.longChoice, "long"))
  const longMarks = longQuestions.slice(0, longRequired).reduce((sum, q) => sum + (q.marks || 0), 0)

  return Math.max(0, mcqsMarks + shortMarks + longMarks)
}

// Update the calculatePaperTotalMarks function as well
const calculatePaperTotalMarks = () => {
  let totalMarks = 0

  // Calculate marks for short questions
  const shortQuestions = selectedQuestions.filter((q) => q.type === "short")
  const shortMarks = Math.max(0, calculateMarksForQuestionType(shortQuestions, "short"))
  totalMarks += shortMarks

  // Calculate marks for long questions
  const longQuestions = selectedQuestions.filter((q) => q.type === "long")
  const longMarks = Math.max(0, calculateMarksForQuestionType(longQuestions, "long"))
...
// Update the determineOptionLayout function to handle Urdu text in options
const determineOptionLayout = (options, hasParts) => {
  ...
  // Check if any option contains Urdu text
  const hasUrduOptions = options.some((option) => isUrduText(option))
        ...
        {options.map((option, idx) => {
          ...
          return (
            <div key={idx} className="flex items-center gap-2">
              <span className="font-medium w-6 text-right">({String.fromCharCode(65 + idx)})</span>
              <span 
...
// Update the renderQuestionParts function to directly apply Nastaliq font:
  ...
  return (
    <div className="ml-6 mt-2">
      {question.parts.map((part, idx) => {
        const isPartUrdu = isUrduText(part);
        ...
        return (
          <div
            ...
            style={{
              ...
              marginBottom: idx === 1 ? "0.9rem" : `${paperSettings.longQuestionPartsLineHeight * 0.5}rem`,
            }}
          >
            <div className="flex-shrink-0">
              <span className="font-medium mr-3 inline-block">{String.fromCharCode(65 + idx)}.</span>
            </div>
                  ...
                  <div className="grid grid-cols-3 gap-6">
                    <div className="flex items-center gap-3">
                      ...
                      >
                        ...
                        <option value="sq_book_i">Short Questions (Book I)</option>
                        <option value="sqBoPlays">Short Questions (Book III Plays)</option>
                        <option value="sqBokPoems">Short Questions (Book III Poems)</option>
                        <option value="letter">Letter</option>
                        <option value="application">Application</option>
                        <option value="story">Story</option>
                        <option value="exp_poems">Poem Translation</option>
                        <option value="punctuation">Punctuation</option>
                        <option value="pair_o_w">Pair of Words</option>
                        <option value="e_to_u">English to Urdu</option>
                      </select>
                    </div>
...
<div className="grid grid-cols-4 gap-4">
  <div className="flex items-center gap-2">
    <div className="bg-violet-100 text-violet-700 px-3 py-2 text-xs rounded-lg font-medium shadow-sm whitespace-nowrap">
      MCQs Choice
    </div>
    <input
      type="number"
      min="0"
      value={paperSettings.mcqsChoice}
      onChange={(e) => handleMcqsChoiceChange(e.target.value)}
  ...
  <div className="flex items-center gap-2">
    ...
    <input
      type="number"
      min="0"
      value={paperSettings.sq_book_iChoice}
      onChange={(e) => handleSqBookIChoiceChange(e.target.value)}
      className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
    />
  </div>
  <div className="flex items-center gap-2">
    ...
    <input
      type="number"
      min="0"
      value={paperSettings.sqBoPlaysChoice}
      onChange={(e) => handleSqBoPlaysChoiceChange(e.target.value)}
      className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
    />
  </div>
  <div className="flex items-center gap-2">
    ...
    <input
      type="number"
      min="0"
      value={paperSettings.sqBokPoemsChoice}
      onChange={(e) => handleSqBokPoemsChoiceChange(e.target.value)}
      className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
    />
  </div>
  <div className="flex items-center gap-2">
    ...
    <input
      type="number"
      min="0"
      value={paperSettings.letterChoice}
      onChange={(e) => handleLetterChoiceChange(e.target.value)}
      className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
    />
  </div>
  <div className="flex items-center gap-2">
    ...
    <input
      type="number"
      min="0"
      value={paperSettings.applicationChoice}
      onChange={(e) => handleApplicationChoiceChange(e.target.value)}
      className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
    />
  </div>
  <div className="flex items-center gap-2">
    ...
    <input
      type="number"
      min="0"
      value={paperSettings.storyChoice}
      onChange={(e) => handleStoryChoiceChange(e.target.value)}
      className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
    />
  </div>
  <div className="flex items-center gap-2">
    ...
    <input
      type="number"
      min="0"
      value={paperSettings.exp_poemsChoice}
      onChange={(e) => handleExpPoemsChoiceChange(e.target.value)}
      className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
    />
  </div>
  <div className="flex items-center gap-2">
    ...
    <input
      type="number"
      min="0"
      value={paperSettings.punctuationChoice}
      onChange={(e) => handlePunctuationChoiceChange(e.target.value)}
      className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
    />
  </div>
  <div className="flex items-center gap-2">
    ...
    <input
      type="number"
      min="0"
      value={paperSettings.pair_o_wChoice}
      onChange={(e) => handlePairOWChoiceChange(e.target.value)}
      className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
    />
  </div>
  <div className="flex items-center gap-2">
    ...
    <input
      type="number"
      min="0"
      value={paperSettings.e_to_uChoice}
      onChange={(e) => handleEToUChoiceChange(e.target.value)}
      className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
    />
  </div>
</div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p
                ...
                style={isUrduText(question.text) ? {
                  fontFamily: `${nastaliq.style.fontFamily}, sans-serif`,
                  fontSize: `${paperSettings.textFontSize}px`,
                  color: paperSettings.textFontColor || "black",
                  lineHeight: paperSettings.lineHeight || "1.5",
                  direction: 'rtl'
                } : getQuestionTextStyle(question.type, question.text)}
              >
                {question.text}
              </p>
                    ...
                    question.options.map((option, idx) => {
                      ...
                      return (
                        <div key={idx} className="flex items-start mb-1">
                          <span className="font-medium w-6 text-right mr-2">({String.fromCharCode(65 + idx)})</span>
                          <span
                            style={isOptionUrdu ? { 
                              fontFamily: `${nastaliq.style.fontFamily}, sans-serif`,
                              fontSize: `${paperSettings.textFontSize}px`,
...
<div className="mt-3">
  ...
  <p className="text-xs text-gray-500 mt-1">
    Affects MCQ question text spacing
  </p>
</div>
...
<div className="mt-3">
  ...
  <input
    type="number"
    step="0.1"
    value={paperSettings.essayLineHeight}
    onChange={(e) =>
      setPaperSettings((prev) => ({
        ...prev,
  ...
  />
  <p className="text-xs text-gray-500 mt-1">
    Affects essay question text spacing
  </p>
</div>
...
<div className="mt-3">
  ...
  <input
    type="number"
    step="0.1"
    value={paperSettings.storyLineHeight}
    onChange={(e) =>
      setPaperSettings((prev) => ({
        ...prev,
  ...
  />
  <p className="text-xs text-gray-500 mt-1">
    Affects story question text spacing
  </p>
</div>
...
<div className="mt-3">
  ...
  <input
    type="number"
    step="0.1"
    value={paperSettings.dialogueLineHeight}
    onChange={(e) =>
      setPaperSettings((prev) => ({
        ...prev,
  ...
  />
  <p className="text-xs text-gray-500 mt-1">
    Affects dialogue question text spacing
  </p>
</div>
...
<div className="mt-3">
  ...
  <input
    type="number"
    step="0.1"
    value={paperSettings.applicationLineHeight}
    onChange={(e) =>
      setPaperSettings((prev) => ({
        ...prev,
  ...
  />
  <p className="text-xs text-gray-500 mt-1">
    Affects application question text spacing
  </p>
</div>
...
<div className="mt-3">
  ...
  <input
    type="number"
    step="0.1"
    value={paperSettings.e_to_uLineHeight || paperSettings.lineHeight}
    onChange={(e) =>
      setPaperSettings((prev) => ({
        ...prev,
        e_to_uLineHeight: e.target.value,
      }))
    }
    className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30"
  />
...
<div className="mt-3">
  ...
  <input
    type="number"
    step="0.1"
    value={paperSettings.u_to_eLineHeight || paperSettings.lineHeight}
    onChange={(e) =>
      setPaperSettings((prev) => ({
        ...prev,
        u_to_eLineHeight: e.target.value,
      }))
    }
    className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30"
  />
...
<div className="mt-3">
  ...
  <input
    type="number"
    step="0.1"
    value={paperSettings.dir_indLineHeight || paperSettings.lineHeight}
    onChange={(e) =>
      setPaperSettings((prev) => ({
        ...prev,
        dir_indLineHeight: e.target.value,
      }))
    }
    className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30"
  />
...
<div className="mt-3">
  <label className="block text-sm text-gray-600 mb-1">
    Pair of Words Line Height
  </label>
  <input
    type="number"
    step="0.1"
    value={paperSettings.pair_o_wLineHeight || paperSettings.lineHeight}
    onChange={(e) =>
      setPaperSettings((prev) => ({
        ...prev,
        pair_o_wLineHeight: e.target.value,
      }))
    }
    className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30"
  />
            ...
            {(() => {
              ...
              return Object.entries(sections).map(([type, typeSections]) =>
                typeSections.map((section, sectionIndex) => {
                  ...
                  const marksNumber =
                    type === "short" || type === "long" || type === "sq_book_i" || type === "sqBoPlays" || type === "sqBokPoems"  || type === "letter"  || type === "application"  || type === "story"   || type === "exp_poems"   || type === "punctuation"   || type === "pair_o_w"    || type === "e_to_u"
