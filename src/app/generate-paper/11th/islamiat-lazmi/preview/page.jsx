...
export default function PaperPreview() {
  ...
  const [sections, setSections] = useState({
    ...
    hadith: [{ title: "حدیث کا ترجمہ", questions: [], choice: 0 }],
  ...
  useEffect(() => {
    ...
    const savedExercises = localStorage.getItem("selectedIslamiyatExercises");
    if (savedExercises) {
      try {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const subject = paperSettings.subject || "Islamiat"; // Get current subject
      ...
      if (savedSettings) {
        try {
          ...
          setPaperSettings((prev) => ({
            ...prev,
            ...parsedSettings,
            subject: "Islamiat", // Ensure subject is set to Islamiat
          }));
  ...
  const totalMarks = useMemo(() => {
    ...
    Object.entries(sections).forEach(([type, typeSections]) => {
      typeSections.forEach((section) => {
        ...
        if (type === "short" || type === "long" || type === "hadith" || type === "note" || type === "ayat") {
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const params = new URLSearchParams({
          class: "11th",
          subject: "Islamiat", // Use Islamiat for the fetch
        });
  ...
  if (!isClient) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100" style={paperContainerStyle}>
...
setSections((prevSections) => {
  ...
  Object.entries(questionsByType).forEach(([type, questions]) => {
    ...
    switch (type) {
      ...
      case "long":
        ...
        break;
      case "hadith":
        choiceValue = Math.min(paperSettings.hadithChoice || 0, questions.length - 1);
        break;
      case "ayat":
        choiceValue = Math.min(paperSettings.ayatChoice || 0, questions.length - 1);
        break;
      case "note":
        choiceValue = Math.min(paperSettings.noteChoice || 0, questions.length - 1);
        break;
      default:
        choiceValue = 0;
    }
  ...
  const handlePrintClick = () => {
    ...
    const filename = `11th-islamiat-${paperSettings.section || "section"}-${
...
`);
  ...
  const handleSavePaper = async () => {
    try {
      ...
      const paperData = {
        ...
        subject: paperSettings.subject || "Islamiat",
  ...
  const groupQuestionsByType = (questions) => {
    // First, group questions by their type (mcqs, short, long, hadith, note)
...
const getHeadingTextByType = (type, section) => {
  ...
  switch (type) {
    ...
    case "long":
      ...
      break;
    case "hadith":
      choiceValue = paperSettings.hadithChoice || 0;
      break;
    case "ayat":
  ...
  switch (type) {
    ...
    case "hadith":
      return `حدیث کا ترجمہ (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
    case "ayat":
...
const calculateMarksForQuestionType = (questions, type) => {
  ...
  switch (type) {
    ...
    case "long":
      ...
      break;
    case "hadith":
      choice = paperSettings.hadithChoice || 0;
      break;
    case "ayat":
  ...
  if (["short", "long", "hadith", "ayat", "note"].includes(type)) {
...
const calculateTotalMarks = () => {
  ...
  const hadithMarks = calculateTypeMarks("hadith", paperSettings.hadithChoice);
  ...
  return mcqsMarks + shortMarks + longMarks + hadithMarks + ayatMarks + noteMarks;
};
...
const calculatePaperTotalMarks = () => {
  ...
  const questionTypes = ["mcqs", "short", "long", "hadith", "ayat", "note"];
...
// ... existing code ...
  ...
  const determineOptionLayout = (options, hasParts) => {
    ...
    if (hasVeryLongOptions) {
      ...
      return (
        <div className="grid grid-cols-2 gap-x-8 gap-y-1 max-w-2xl text-right" dir="rtl">
          {options.map((option, idx) => (
            <div key={idx} className="flex items-center gap-2 flex-row-reverse">
              <span className="font-medium w-6 text-right">
                ({String.fromCharCode(65 + idx)})
  ...
  return (
    <ProtectedRoute>
      <div className={`min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 ${nastaliq.variable}`} style={paperContainerStyle}>
        {/* Navigation Bar */}
        ...
        {/* Question Menu Modal */}
        {showQuestionMenu && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-gradient-to-br from-gray-50/95 to-slate-100/95 backdrop-blur-sm" dir="rtl">
            <div className="bg-white rounded-xl w-[98%] max-w-[1920px] shadow-2xl border border-violet-100" dir="ltr">
              {/* Header */}
              <div className="bg-gradient-to-r from-violet-600 to-indigo-500 text-white px-4 py-2.5 flex justify-between items-center rounded-t-xl" dir="rtl">
                  ...
                  <div className="grid grid-cols-3 gap-6">
                    <div className="flex items-center gap-3">
                      ...
                      >
                        ...
                        <option value="hadith">Hadith Translation (حدیث کا ترجمہ)</option>
                        <option value="ayat">Ayat Translation (آیات کا ترجمہ)</option>
                  ...
                  <div className="grid grid-cols-3 gap-6">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <input
                        type="number"
                        min="0"
                        value={paperSettings.hadithChoice || 0}
                        onChange={(e) =>
                          setPaperSettings((prev) => ({
                            ...prev,
                            hadithChoice: parseInt(e.target.value) || 0,
                          }))
                        }
                        className="w-full border border-violet-100 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
                      />
        ...
        {/* Edit Panel */}
        {showEditPanel && (
          <div className="fixed right-0 top-0 h-screen w-96 bg-white shadow-2xl overflow-y-auto z-40 text-left" dir="ltr">
            <div className="sticky top-0 bg-gradient-to-r from-violet-600 to-indigo-500 text-white p-4 flex justify-between items-center text-left" dir="ltr">
              <h3 className="text-lg font-semibold text-left">Paper Settings</h3>
              <button
          ...
          <div className="bg-white rounded-lg shadow-xl border border-gray-200 print-content paper-container">
            ...
            >
              ...
              {paperSettings.paperHeaderLayout === "LEAP Header" && (
                <>
                  <div className="flex items-center justify-center p-2 border-b border-gray-200 text-left" dir="ltr">
                    <DynamicImage
                      src="/leap-header.png"
            ...
            {(() => {
              ...
              return Object.entries(sections).map(([type, typeSections]) =>
                typeSections.map((section, sectionIndex) => {
                  ...
                  const marksNumber =
                    type === "short" || type === "ayat" || type === "hadith" || type === "long" || type === "note"
