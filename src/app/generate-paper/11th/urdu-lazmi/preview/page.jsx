...
// Function to calculate required questions based on choice count
const calculateRequiredQuestions = (questions, choiceCount, type) => {
  const typeQuestions = questions.filter((q) => q.type === type)
  return choiceCount > 0 ? typeQuestions.length - choiceCount : typeQuestions.length
}
...
// Device identifier function for settings persistence
...
export default function PaperPreview() {
  ...
  const [medium, setMedium] = useState("URDU MEDIUM"); // Default to Urdu for Islamiyat
  const [choiceNumber, setChoiceNumber] = useState(0);

  // Define sections with Islamiyat-specific question types
  const [sections, setSections] = useState({
    mcqs: [{ title: "ایم سی کیوز", questions: [], choice: 0 }],
    poet_ex: [{ title: "اشعار کی تشریح حصہ نظم", questions: [], choice: 0 }],
    ghazl_ex: [{ title: "اشعار کی تشریح حصہ غزل", questions: [], choice: 0 }],
    proseExp: [{ title: "نثر پاروں کی تشریح", questions: [], choice: 0 }],
    poem_summ: [{ title: "نظم کا خلاصہ", questions: [], choice: 0 }],
    lessonSumm: [{ title: "سبق کا خلاصہ", questions: [], choice: 0 }],
    dialogue: [{ title: "مکالمہ", questions: [], choice: 0 }],
    report: [{ title: "روداد", questions: [], choice: 0 }],
    receipt: [{ title: "رسید", questions: [], choice: 0 }],
    application: [{ title: "درخواست", questions: [], choice: 0 }],
    precis: [{ title: "تلخیص", questions: [], choice: 0 }]
  });
  ...
  // Paper settings with Islamiyat defaults
  const paperContainerStyle = {
    direction: "rtl",
    fontFamily: nastaliq.className,
    textAlign: "right",
  };

  const [paperSettings, setPaperSettings] = useState({
    paperHeaderLayout: "LEAP Header",
    headerFontStyle: nastaliq.className,
    headerFontSize: "18",
    headerFontColor: "Black",
    headingFontStyle: nastaliq.className,
    headingFontSize: "16",
    ...
    urduTextFontStyle: nastaliq.className,
    lineHeight: "2.0",
    shortQuestionLineHeight: "2.0",
    longQuestionLineHeight: "2.0",

    q_w_mLineHeight: "2.0", // Added for words meaning
    ayatLineHeight: "2.0", // Added for ayat translation
    noteLineHeight: "2.0", // Added for notes
    ...
    longQuestionPartsLineHeight: 2.0,
    ...
    poet_exLineHeight: "2.0",
    ghazl_exLineHeight: "2.0",
    proseExpLineHeight: "2.0",
    poem_summLineHeight: "2.0",
    lessonSummLineHeight: "2.0",
    dialogueLineHeight: "2.0",
    reportLineHeight: "2.0",
    receiptLineHeight: "2.0",
    applicationLineHeight: "2.0",
    precisLineHeight: "2.0",
    summaryLineHeight: "2.0",
    cen_ideaLineHeight: "2.0",
    letterLineHeight: "2.0",
    storyLineHeight: "2.0",
    sen_corrLineHeight: "2.0",
    provrb_expLineHeight: "2.0",
  });
  ...
  const handleShortChoiceChange = (value) => {
    setPaperSettings((prev) => ({
      ...prev,
      shortChoice: Math.max(0, Number.parseInt(value) || 0),
    }));
  };
  ...
  const handleLongChoiceChange = (value) => {
    setPaperSettings((prev) => ({
      ...prev,
      longChoice: Math.max(0, Number.parseInt(value) || 0),
    }));
  };

  // ... existing code ...

  const handleQwmLineHeightChange = (value) => {
    setPaperSettings((prev) => ({
      ...prev,
      q_w_mLineHeight: value,
    }));
  };

  const handleAyatLineHeightChange = (value) => {
    setPaperSettings((prev) => ({
      ...prev,
      ayatLineHeight: value,
    }));
  };

  const handleNoteLineHeightChange = (value) => {
    setPaperSettings((prev) => ({
      ...prev,
      noteLineHeight: value,
    }));
  };
  ...
  useEffect(() => {
    ...
    const savedExercises = localStorage.getItem("selectedUrduLazmi11Exercises");
    if (savedExercises) {
      try {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const subject = paperSettings.subject || "Urdu"; // Get current subject
      ...
      if (savedSettings) {
        try {
          ...
          setPaperSettings((prev) => ({
            ...prev,
            ...parsedSettings,
            subject: "Urdu", // Ensure subject is set to Islamiat
          }));
  ...
  // Get marks per question type for Islamiyat
  const getMarksPerQuestion = (type) => {
    switch (type) {
      case "mcqs":
        return 1; // MCQs - 1 mark
      case "short":
        return 2; // Short questions - 2 marks
      case "poet_ex":
      case "ghazl_ex":
      case "proseExp":
      case "poem_summ":
      ...
      case "receipt":
      case "precis":
        return 5; // 5 marks questions
      case "application":
        return 10; // 10 marks questions
      default:
        return 0;
    }
  };
...
const preserveLineBreaks = (text, type) => {
  let lineHeight = paperSettings.lineHeight || "2.0"; // default from paper settings
  const marginBottom = "0"; // default margin

  // Get the appropriate line height based on question type
  switch (type) {
  case "mcqs":
  ...
  case "short":
    ...
    break;
  case "poet_ex":
    lineHeight = paperSettings.poet_exLineHeight;
    break;
  case "ghazl_ex":
    lineHeight = paperSettings.ghazl_exLineHeight;
    break;
  case "proseExp":
    lineHeight = paperSettings.proseExpLineHeight;
    break;
  case "poem_summ":
    lineHeight = paperSettings.poem_summLineHeight;
    break;
  case "lessonSumm":
    lineHeight = paperSettings.lessonSummLineHeight;
    break;
  case "dialogue":
    lineHeight = paperSettings.dialogueLineHeight;
    break;
  case "report":
    lineHeight = paperSettings.reportLineHeight;
    break;
  case "receipt":
    lineHeight = paperSettings.receiptLineHeight;
    break;
  case "application":
    lineHeight = paperSettings.applicationLineHeight;
    break;
  case "precis":
    lineHeight = paperSettings.precisLineHeight;
    break;
  default:
    lineHeight = paperSettings.lineHeight;
}
  ...
  return {
    ...
    fontFamily: nastaliq.className // ensure Urdu font is applied
  };
};
...
// ... existing code ...
  ...
  // Update maximum marks when total marks change
  useEffect(() => {
    setPaperSettings((prev) => ({
      ...prev,
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const params = new URLSearchParams({
          class: "11th",
          subject: "Urdu", // Use Islamiat for the fetch
        });
  ...
  switch (questionType) {
    ...
    case "q_w_m":
      return {
        ...baseStyle,
        lineHeight: paperSettings.q_w_mLineHeight,
      };
    case "ayat":
      return {
        ...baseStyle,
        lineHeight: paperSettings.ayatLineHeight,
      };
    case "note":
      return {
        ...baseStyle,
        lineHeight: paperSettings.noteLineHeight,
      };
    case "poet_ex":
      return {
        ...baseStyle,
        lineHeight: paperSettings.poet_exLineHeight || paperSettings.lineHeight,
      };
    case "ghazl_ex":
      return {
        ...baseStyle,
        lineHeight: paperSettings.ghazl_exLineHeight || paperSettings.lineHeight,
      };
    case "proseExp":
      return {
        ...baseStyle,
        lineHeight: paperSettings.proseExpLineHeight || paperSettings.lineHeight,
      };
    case "poem_summ":
      return {
        ...baseStyle,
        lineHeight: paperSettings.poem_summLineHeight || paperSettings.lineHeight,
      };
    case "lessonSumm":
      return {
        ...baseStyle,
        lineHeight: paperSettings.lessonSummLineHeight || paperSettings.lineHeight,
      };
    case "dialogue":
      return {
        ...baseStyle,
        lineHeight: paperSettings.dialogueLineHeight || paperSettings.lineHeight,
      };
    case "report":
      return {
        ...baseStyle,
        lineHeight: paperSettings.reportLineHeight || paperSettings.lineHeight,
      };
    case "receipt":
      return {
        ...baseStyle,
        lineHeight: paperSettings.receiptLineHeight || paperSettings.lineHeight,
      };
    case "application":
      return {
        ...baseStyle,
        lineHeight: paperSettings.applicationLineHeight || paperSettings.lineHeight,
      };
    case "precis":
      return {
        ...baseStyle,
        lineHeight: paperSettings.precisLineHeight || paperSettings.lineHeight,
      };
    default:
  ...
  const handleAddSelection = () => {
    ...
    // Inside handleAddSelection function
    setSections((prevSections) => {
      ...
      Object.entries(questionsByType).forEach(([type, questions]) => {
        ...
        let choiceValue = 0;
        // Add choice handling for all question types
        switch (type) {
          case "poet_ex":
            choiceValue = Math.min(
              paperSettings.poet_exChoice || 0,
              questions.length - 1
            );
            break;
          case "ghazl_ex":
            choiceValue = Math.min(
              paperSettings.ghazl_exChoice || 0,
              questions.length - 1
            );
            break;
          case "proseExp":
            choiceValue = Math.min(
              paperSettings.proseExpChoice || 0,
              questions.length - 1
            );
            break;
          case "poem_summ":
            choiceValue = Math.min(
              paperSettings.poem_summChoice || 0,
              questions.length - 1
            );
            break;
          case "lessonSumm":
            choiceValue = Math.min(
              paperSettings.lessonSummChoice || 0,
              questions.length - 1
            );
            break;
          case "dialogue":
            choiceValue = Math.min(
              paperSettings.dialogueChoice || 0,
              questions.length - 1
            );
            break;
          case "report":
            choiceValue = Math.min(
              paperSettings.reportChoice || 0,
              questions.length - 1
            );
            break;
          case "receipt":
            choiceValue = Math.min(
              paperSettings.receiptChoice || 0,
              questions.length - 1
            );
            break;
          case "application":
            choiceValue = Math.min(
              paperSettings.applicationChoice || 0,
              questions.length - 1
            );
            break;
          case "precis":
            choiceValue = Math.min(
              paperSettings.precisChoice || 0,
              questions.length - 1
            );
            break;
          default:
        ...
        // Set title based on question type in Urdu with choice information
        let title = "";
        switch (type) {
          case "mcqs":
          ...
          case "poet_ex":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} اشعار کی تشریح کیجیے۔`
              : "اشعار کی تشریح حصہ نظم";
            break;
          case "ghazl_ex":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} اشعار کی تشریح کیجیے۔`
              : "اشعار کی تشریح حصہ غزل";
            break;
          case "proseExp":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} نثر پاروں کی تشریح کیجیے۔`
              : "نثر پاروں کی تشریح";
            break;
          case "poem_summ":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} نظم کا خلاصہ تحریر کیجیے۔`
              : "نظم کا خلاصہ";
            break;
          case "lessonSumm":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} سبق کا خلاصہ تحریر کیجیے۔`
              : "سبق کا خلاصہ";
            break;
          case "dialogue":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} مکالمہ تحریر کیجیے۔`
              : "مکالمہ";
            break;
          case "report":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} روداد تحریر کیجیے۔`
              : "روداد";
            break;
          case "receipt":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} رسید تحریر کیجیے۔`
              : "رسید";
            break;
          case "application":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} درخواست تحریر کیجیے۔`
              : "درخواست";
            break;
          case "precis":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} تلخیص تحریر کیجیے۔`
              : "تلخیص";
            break;
          default:
            title = `ATTEMPT ANY ${requiredQuestions} OF THE FOLLOWING ${questions.length} QUESTIONS`;
        }
        const newSection = {
  ...
  const handlePrintClick = () => {
    ...
    const filename = `11th-Urdu-${paperSettings.section || "section"}-${
...
body {
  ...
  font-family: font-nastaliq !important;
...
/* Header styles - keep LTR */
.paper-container > div:first-child {
  direction: ltr !important;
}
/* Content styles - ensure RTL */
.mt-8.pl-16 {
...
@media print {
  ...
  * {
  ...
  /* Ensure MCQ options are properly aligned in RTL */
  .grid.grid-cols-4,
  .grid.grid-cols-2,
...
`);
  ...
  const handleSavePaper = async () => {
    try {
      ...
      const paperData = {
        class: paperSettings.class || "11th",
        subject: paperSettings.subject || "Urdu",
  ...
  const groupQuestionsByType = (questions) => {
    // First, group questions by their type (mcqs, short, long, q_w_m, note)
  ...
  // Get the appropriate choice value based on question type
  switch (type) {
    case "short":
      choiceValue = paperSettings.shortChoice || 0;
      break;
    case "poet_ex":
      choiceValue = paperSettings.poet_exChoice || 0;
      break;
    case "ghazl_ex":
      choiceValue = paperSettings.ghazl_exChoice || 0;
      break;
    case "proseExp":
      choiceValue = paperSettings.proseExpChoice || 0;
      break;
    case "poem_summ":
      choiceValue = paperSettings.poem_summChoice || 0;
      break;
    case "lessonSumm":
      choiceValue = paperSettings.lessonSummChoice || 0;
      break;
    case "dialogue":
      choiceValue = paperSettings.dialogueChoice || 0;
      break;
    case "report":
      choiceValue = paperSettings.reportChoice || 0;
      break;
    case "receipt":
      choiceValue = paperSettings.receiptChoice || 0;
      break;
    case "application":
      choiceValue = paperSettings.applicationChoice || 0;
      break;
    case "precis":
      choiceValue = paperSettings.precisChoice || 0;
      break;
  }

  const requiredQuestions = choiceValue > 0 ? totalQuestions - choiceValue : totalQuestions;
  const totalMarks = requiredQuestions * marksPerQuestion;

  switch (type) {
    case "mcqs":
      return `ایم سی کیوز (${totalQuestions}×1=${totalQuestions})`;
    case "short":
      return `مختصر سوالات (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
    case "poet_ex":
      return `اشعار کی تشریح حصہ نظم (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
    case "ghazl_ex":
      return `اشعار کی تشریح حصہ غزل (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
    case "proseExp":
      return `نثر پاروں کی تشریح (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
    case "poem_summ":
      return `نظم کا خلاصہ (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
    case "lessonSumm":
      return `سبق کا خلاصہ (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
    case "dialogue":
      return `مکالمہ (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
    case "report":
      return `روداد (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
    case "receipt":
      return `رسید (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
    case "application":
      return `درخواست (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
    case "precis":
      return `تلخیص (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
    default:
      return "";
  }
};
  ...
  const calculateMarksForQuestionType = (questions, type) => {
    const totalQuestions = questions.length;
    const marksPerQuestion = getMarksPerQuestion(type);
    let choice = 0;

    // Get the appropriate choice based on question type
    switch (type) {
      case "poet_ex":
        choice = paperSettings.poet_exChoice || 0;
        break;
      case "ghazl_ex":
        choice = paperSettings.ghazl_exChoice || 0;
        break;
      case "proseExp":
        choice = paperSettings.proseExpChoice || 0;
        break;
      case "poem_summ":
        choice = paperSettings.poem_summChoice || 0;
        break;
      case "lessonSumm":
        choice = paperSettings.lessonSummChoice || 0;
        break;
      case "dialogue":
        choice = paperSettings.dialogueChoice || 0;
        break;
      case "report":
        choice = paperSettings.reportChoice || 0;
        break;
      case "receipt":
        choice = paperSettings.receiptChoice || 0;
        break;
      case "application":
        choice = paperSettings.applicationChoice || 0;
        break;
      case "precis":
        choice = paperSettings.precisChoice || 0;
        break;
    }

    // Calculate marks based on choice
    if (type !== "mcqs") {
      const questionsToAttempt =
        choice > 0 ? totalQuestions - choice : totalQuestions;
      return Math.min(required, questions.length) * getMarksPerQuestion(type);
    }

    // For MCQs, return all marks as there's no choice
  ...
  // Calculate marks for each question type with choice
  const calculateTypeMarks = (type, choice) => {
    const questions = selectedQuestions.filter((q) => q.type === type);
    const required = choice > 0 ? questions.length - choice : questions.length;
    return Math.min(required, questions.length) * getMarksPerQuestion(type);
  };

  const shortMarks = calculateTypeMarks("short", paperSettings.shortChoice);
  const poet_exMarks = calculateTypeMarks("poet_ex", paperSettings.poet_exChoice);
  const ghazl_exMarks = calculateTypeMarks("ghazl_ex", paperSettings.ghazl_exChoice);
  const proseExpMarks = calculateTypeMarks("proseExp", paperSettings.proseExpChoice);
  const poem_summMarks = calculateTypeMarks("poem_summ", paperSettings.poem_summChoice);
  const lessonSummMarks = calculateTypeMarks("lessonSumm", paperSettings.lessonSummChoice);
  const dialogueMarks = calculateTypeMarks("dialogue", paperSettings.dialogueChoice);
  const reportMarks = calculateTypeMarks("report", paperSettings.reportChoice);
  const receiptMarks = calculateTypeMarks("receipt", paperSettings.receiptChoice);
  const applicationMarks = calculateTypeMarks("application", paperSettings.applicationChoice);
  const precisMarks = calculateTypeMarks("precis", paperSettings.precisChoice);

  return (
    mcqsMarks + shortMarks + poet_exMarks + ghazl_exMarks + proseExpMarks + 
    poem_summMarks + lessonSummMarks + dialogueMarks + reportMarks + 
    receiptMarks + applicationMarks + precisMarks
  );
};
  ...
  const questionTypes = [
    "mcqs", "short", "poet_ex", "ghazl_ex", "proseExp", "poem_summ", 
    "lessonSumm", "dialogue", "report", "receipt", "application", "precis"
  ];

  questionTypes.forEach((type) => {
    const questions = selectedQuestions.filter((q) => q.type === type);
    const choice = type === "mcqs" ? 0 : paperSettings[`${type}Choice`] || 0;
    const marks = calculateMarksForQuestionType(questions, type, choice);
    totalMarks += marks;
  });
  ...
  const determineOptionLayout = (options, hasParts) => {
    ...
    if (hasVeryLongOptions) {
      ...
      return (
        <div className="flex flex-col space-y-1 max-w-2xl text-right" dir="rtl">
      ...
      // Grid layout (2x2) for moderately long options - adjusted to be much more right-aligned
      return (
        <div
          className="grid grid-cols-2 gap-x-8 gap-y-1 max-w-2xl text-right"
          dir="rtl"
        >
          {options.map((option, idx) => (
            <div key={idx} className="flex items-center gap-2 flex-row-reverse">
      ...
      return (
        <div className="grid grid-cols-4 gap-2 max-w-3xl text-right" dir="rtl">
          {options.map((option, idx) => (
            <div key={idx} className="flex items-center gap-2 flex-row-reverse">
              <span className="font-medium w-6 text-right">
                ({String.fromCharCode(65 + idx)})
  ...
  const renderQuestionParts = (question) => {
    ...
    return (
      <div className={`mr-6 ${partsMargin}`} dir="rtl">
        {question.parts.map((part, idx) => (
          ...
          >
            ...
            >
              <span className="font-medium ml-3 inline-block">
                {idx === 0
                  ? "الف."
                  : idx === 1
                  ? "ب."
  ...
  return (
    <ProtectedRoute>
      <div
        className={`min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 ${nastaliq.variable}`}
        style={paperContainerStyle}
      >
        {/* Navigation Bar */}
        <div className="bg-white shadow-lg p-2">
          <div className="max-w-7xl mx-auto flex gap-2 " dir="ltr">
            <div
        ...
        {showQuestionMenu && (
          <div
            className="fixed inset-0 z-50 flex items-center justify-center bg-gradient-to-br from-gray-50/95 to-slate-100/95 backdrop-blur-sm"
            dir="rtl"
          >
            <div
              className="bg-white rounded-xl w-[98%] max-w-[1920px] shadow-2xl border border-violet-100"
              dir="ltr"
            >
              {/* Header */}
              <div
                className="bg-gradient-to-r from-violet-600 to-indigo-500 text-white px-4 py-2.5 flex justify-between items-center rounded-t-xl"
                dir="rtl"
              >
              ...
              <div className="p-4 bg-gradient-to-br from-gray-50 to-white">
                {/* Control Panel */}
                <div className="space-y-4 bg-white p-6 rounded-xl shadow-sm border border-violet-100">
  {/* Row 1: Question Type, Selection, Required */}
  <div className="grid grid-cols-3 gap-6">
    <div className="flex items-center gap-3">
      <div className="bg-gradient-to-r from-violet-600 to-indigo-500 text-white px-4 py-2.5 text-sm rounded-lg font-medium shadow-sm whitespace-nowrap  ">
        Question Type
      </div>
      <select
        value={questionType}
        onChange={(e) => setQuestionType(e.target.value)}
        className={`w-full border border-violet-100 rounded-lg px-4 py-0.3 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30 font-nastaliq`}
      >
        <option className="font-nastaliq" value="">Select Type</option>
        <option className="font-nastaliq" value="mcqs">ایم سی کیوز</option>
        <option className="font-nastaliq" value="poet_ex">اشعار کی تشریح حصہ نظم</option>
        <option className="font-nastaliq" value="ghazl_ex">اشعار کی تشریح حصہ غزل</option>
        <option className="font-nastaliq" value="proseExp">نثر پاروں کی تشریح</option>
        <option className="font-nastaliq" value="poem_summ">نظم کا خلاصہ</option>
        <option className="font-nastaliq" value="lessonSumm">سبق کا خلاصہ</option>
        <option className="font-nastaliq" value="dialogue">مکالمہ</option>
        <option className="font-nastaliq" value="report">روداد</option>
        <option className="font-nastaliq" value="receipt">رسید</option>
        <option className="font-nastaliq" value="application">درخواست</option>
        <option className="font-nastaliq" value="precis">تلخیص</option>
      </select>
    </div>
  ...
  <div className="mt-4">
  <div className={`bg-gradient-to-r from-violet-600 to-indigo-500 text-white px-4 py-2 mb-3 text-center rounded-lg font-medium shadow-sm  text-xl`}>
     Questions Choice
    </div>
    <div className="grid grid-cols-4 gap-4">
    <div className="flex items-center gap-2">
      <div className={`bg-violet-100 text-violet-700 px-3 py-2 text-xs rounded-lg font-medium shadow-sm whitespace-nowrap font-nastaliq`}>
        مختصر سوالات
      </div>
      <input
        type="number"
        min="0"
        value={paperSettings.shortChoice || 0}
        onChange={(e) => handleShortChoiceChange(e.target.value)}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
    <div className="flex items-center gap-2">
      <div className={`bg-violet-100 text-violet-700 px-3 py-2 text-xs rounded-lg font-medium shadow-sm whitespace-nowrap font-nastaliq`}>
      ...
      <input
        type="number"
        min="0"
        value={paperSettings.poet_exChoice || 0}
        onChange={(e) => setPaperSettings(prev => ({...prev, poet_exChoice: Number.parseInt(e.target.value) || 0}))}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
    <div className="flex items-center gap-2">
      ...
      <input
        type="number"
        min="0"
        value={paperSettings.ghazl_exChoice || 0}
        onChange={(e) => setPaperSettings(prev => ({...prev, ghazl_exChoice: Number.parseInt(e.target.value) || 0}))}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
    <div className="flex items-center gap-2">
      ...
      <input
        type="number"
        min="0"
        value={paperSettings.proseExpChoice || 0}
        onChange={(e) => setPaperSettings(prev => ({...prev, proseExpChoice: Number.parseInt(e.target.value) || 0}))}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
  </div>
  ...
  <div className="grid grid-cols-4 gap-4 mt-2">
    <div className="flex items-center gap-2">
      ...
      <input
        type="number"
        min="0"
        value={paperSettings.poem_summChoice || 0}
        onChange={(e) => setPaperSettings(prev => ({...prev, poem_summChoice: Number.parseInt(e.target.value) || 0}))}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
    <div className="flex items-center gap-2">
      ...
      <input
        type="number"
        min="0"
        value={paperSettings.lessonSummChoice || 0}
        onChange={(e) => setPaperSettings(prev => ({...prev, lessonSummChoice: Number.parseInt(e.target.value) || 0}))}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
    <div className="flex items-center gap-2">
      ...
      <input
        type="number"
        min="0"
        value={paperSettings.dialogueChoice || 0}
        onChange={(e) => setPaperSettings(prev => ({...prev, dialogueChoice: Number.parseInt(e.target.value) || 0}))}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
    <div className="flex items-center gap-2">
      ...
      <input
        type="number"
        min="0"
        value={paperSettings.reportChoice || 0}
        onChange={(e) => setPaperSettings(prev => ({...prev, reportChoice: Number.parseInt(e.target.value) || 0}))}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
  </div>
  ...
  <div className="grid grid-cols-4 gap-4 mt-2">
    <div className="flex items-center gap-2">
      ...
      <input
        type="number"
        min="0"
        value={paperSettings.receiptChoice || 0}
        onChange={(e) => setPaperSettings(prev => ({...prev, receiptChoice: Number.parseInt(e.target.value) || 0}))}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
    <div className="flex items-center gap-2">
      ...
      <input
        type="number"
        min="0"
        value={paperSettings.applicationChoice || 0}
        onChange={(e) => setPaperSettings(prev => ({...prev, applicationChoice: Number.parseInt(e.target.value) || 0}))}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
    <div className="flex items-center gap-2">
      ...
      <input
        type="number"
        min="0"
        value={paperSettings.precisChoice || 0}
        onChange={(e) => setPaperSettings(prev => ({...prev, precisChoice: Number.parseInt(e.target.value) || 0}))}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
    <div className="flex items-center gap-2">
      ...
      <input
        type="number"
        min="0"
        value={paperSettings.longChoice || 0}
      ...
      />
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <span className="px-2 py-1 bg-violet-50 text-violet-600 rounded text-xs">
                                Marks:{" "}
                                {question.marks ||
                                  getMarksPerQuestion(question.type)}
                              </span>
                            </div>
                            <p
                              style={preserveLineBreaks(question.text, question.type)}
                              className="text-gray-800 mb-2 font-nastaliq text-right"
                            >
                              {question.text}
                            </p>

                            {/* Show parts for long questions */}
                            {(question.type === "long" ||
                              ...
                              question.parts.some(
                                ...
                                <div className="mt-3 mr-4 space-y-2">
                                  <h4 className="text-sm font-medium text-gray-700 text-right font-nastaliq">
                                    حصے:
                                  </h4>
                                  {question.parts.map(
                                    (part, index) =>
                                      ...
                                      part.trim() !== "" && (
                                        <div
                                          key={index}
                                          className={`pr-4 py-2 border-r-2 ${
                                            part === question.answer
                                              ? "border-green-500 bg-green-50"
                                              : "border-gray-200 bg-gray-50"
                                          }`}
                                        >
                                          <p className="text-gray-700 text-right font-nastaliq">
                                            {part}
                                          </p>
                                        </div>
                                      )
                            ...
                            {/* Display MCQ options */}
                            {question.type === "mcqs" && (
                              <div className="mt-1 mr-6 text-right">
        ...
        {showEditPanel && (
          <div
            className="fixed right-0 top-0 h-screen w-96 bg-white shadow-2xl overflow-y-auto z-40 text-left"
            dir="ltr"
          >
            <div
              className="sticky top-0 bg-gradient-to-r from-violet-600 to-indigo-500 text-white p-4 flex justify-between items-center text-left"
              dir="ltr"
            >
            ...
            <div className="p-6 space-y-6" dir="ltr">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
                    >
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <select
                      value={paperSettings.urduTextFontStyle}
                      onChange={(e) =>
                        setPaperSettings((prev) => ({
                          ...prev,
                          urduTextFontStyle: e.target.value,
                        }))
                      }
                      className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
                    >
                      <option value="Nastaliq">Nastaliq</option>
                      <option value="Noto Nastaliq Urdu">
                        Noto Nastaliq Urdu
                      </option>
                      <option value="Jameel Noori Nastaleeq">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  {/* Line Height Controls for All Question Types */}
<div className="mt-6 space-y-4 border-t border-gray-200 pt-4">
  <h5 className="text-sm font-medium text-gray-700">Question Type Line Heights</h5>
  ...
  <div className="grid grid-cols-2 gap-4">
    ...
    <div>
      <label className="block text-xs text-gray-600 mb-1">
        Poetry Explanation Line Height
      </label>
      <input
        type="number"
        step="0.1"
        value={paperSettings.poet_exLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            poet_exLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
    ...
    <div>
      <label className="block text-xs text-gray-600 mb-1">
        Ghazal Explanation Line Height
      </label>
      <input
        type="number"
        step="0.1"
        value={paperSettings.ghazl_exLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            ghazl_exLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
    ...
    <div>
      <label className="block text-xs text-gray-600 mb-1">
        Prose Explanation Line Height
      </label>
      <input
        type="number"
        step="0.1"
        value={paperSettings.proseExpLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            proseExpLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
    ...
    <div>
      ...
      <input
        type="number"
        step="0.1"
        value={paperSettings.summaryLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            summaryLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
    ...
    <div>
      <label className="block text-xs text-gray-600 mb-1">
        Central Idea Line Height
      </label>
      <input
        type="number"
        step="0.1"
        value={paperSettings.cen_ideaLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            cen_ideaLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
    ...
    <div>
      ...
      <input
        type="number"
        step="0.1"
        value={paperSettings.letterLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            letterLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
    ...
    <div>
      ...
      <input
        type="number"
        step="0.1"
        value={paperSettings.applicationLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            applicationLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
    ...
    <div>
      ...
      <input
        type="number"
        step="0.1"
        value={paperSettings.storyLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            storyLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
    ...
    <div>
      ...
      <input
        type="number"
        step="0.1"
        value={paperSettings.dialogueLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            dialogueLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
    ...
    <div>
      <label className="block text-xs text-gray-600 mb-1">
        Sentence Correction Line Height
      </label>
      <input
        type="number"
        step="0.1"
        value={paperSettings.sen_corrLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            sen_corrLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
    ...
    <div>
      <label className="block text-xs text-gray-600 mb-1">
        Proverb Explanation Line Height
      </label>
      <input
        type="number"
        step="0.1"
        value={paperSettings.provrb_expLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            provrb_expLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
    ...
    <div>
      <label className="block text-xs text-gray-600 mb-1">
        Poem Summary Line Height
      </label>
      <input
        type="number"
        step="0.1"
        value={paperSettings.poem_summLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            poem_summLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
    ...
    <div>
      <label className="block text-xs text-gray-600 mb-1">
        Lesson Summary Line Height
      </label>
      <input
        type="number"
        step="0.1"
        value={paperSettings.lessonSummLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            lessonSummLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
    ...
    <div>
      ...
      <input
        type="number"
        step="0.1"
        value={paperSettings.reportLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            reportLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
    ...
    <div>
      ...
      <input
        type="number"
        step="0.1"
        value={paperSettings.receiptLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            receiptLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
    ...
    <div>
      ...
      <input
        type="number"
        step="0.1"
        value={paperSettings.precisLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            precisLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        // Add 1 day to fix the timezone offset issue
          ...
          <div className="bg-white rounded-lg shadow-xl border border-gray-200 print-content paper-container">
            ...
            >
              ...
              {paperSettings.paperHeaderLayout === "LEAP Header" && (
                <>
                  <div
                    className="flex items-center justify-center p-2 border-b border-gray-200 text-left"
                    dir="ltr"
                  >
                    <DynamicImage
                      src="/leap-header.png"
                  ...
                  <div className="p-2 pl-4" dir="ltr" style={getTextStyle()}>
                    <table className="w-full border-collapse bg-white">
                      <tbody>
                        <tr>
                          <td className="border border-gray-200 p-2 w-1/4 bg-gray-50 text-left">
                            <span className="font-semibold text-gray-800 text-sm">
                              Class:{" "}
                            </span>
                            <span className="text-gray-700 text-sm">
                              {paperSettings.class || "11th"}
                            </span>
                          </td>
                        ...
                        <tr>
                          ...
                          <td className="border border-gray-200 p-2 w-1/4 text-left">
                            <span className="font-semibold text-gray-800 text-sm">
                              Section:{" "}
                            </span>
                            <span className="text-gray-700 text-sm">
                                  ...
                                  <div className="flex items-start">
                                    {!hasLeadingNumber && (
                                      ...
                                      style={{
                                        ...
                                        ...preserveLineBreaks(displayText, type)
                                      }}
                                    >
                                      {displayText}
                                    </p>
