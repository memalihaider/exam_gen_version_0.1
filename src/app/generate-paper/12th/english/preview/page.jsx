...
export default function PaperPreview() {
  ...
  // Define sections with English-specific question types
  const [sections, setSections] = useState({
    ...
    sq_ii_pr_i: [
      {
        title: "Write short answers to the following questions (Book II Modern Prose & Heroes Part-I)",
        questions: [],
        marks: 2,
        choice: 0,
      }
    ...
    sq_ii_pr_ii: [
      {
        title: "Write short answers to the following questions (Book II Modern Prose & Heroes Part-II)",
        questions: [],
        marks: 2,
        choice: 0,
      }
    ...
    mr_chips: [
      {
        title: "Write short answers to the following questions (Mr. Chips)",
        questions: [],
        marks: 2,
        choice: 0,
      }
    ...
    e_to_u: [
      {
        title: "Translate the following paragraph into Urdu.",
        questions: [],
        marks: 15,
        choice: 0,
      }
    ...
    essay: [
      {
        title: "Write an essay on any one of the following topics.",
        questions: [],
        marks: 15,
        choice: 0,
      }
    ...
    idioms: [
      {
        title: "Use the following Idioms/Phrasal verbs in your own sentences.",
        questions: [],
        marks: 2,
        choice: 0,
      }
    ...
    u_to_e: [
      {
        title: "Translate the following paragraph into English.",
        questions: [],
        marks: 15,
        choice: 0,
      }
  ...
  // Paper settings with English defaults
  ...
  const [paperSettings, setPaperSettings] = useState({
    ...
    lineHeight: "1.5",
    shortQuestionLineHeight: "1.5",
    longQuestionLineHeight: "1.5",
    ...
    // Line heights for different question types
    mcqsLineHeight: "1.5",
    shortLineHeight: "1.5",
    e_to_uLineHeight: "1.5",
    u_to_eLineHeight: "1.5",
    summaryLineHeight: "1.5",
    essayLineHeight: "1.5",
    dir_indLineHeight: "1.5",
    pair_o_wLineHeight: "1.5",
  });
  ...
  // Get marks per question type for English
  const getMarksPerQuestion = (type) => {
    switch (type) {
      case "mcqs": return 1;
      case "sq_ii_pr_i": return 2;
      case "sq_ii_pr_ii": return 2;
      case "mr_chips": return 2;
      case "e_to_u": return 15;
      case "u_to_e": return 15;
      case "essay": return 15;
      case "idioms": return 2;
      default: return 0;
    }
  };
  ...
  useEffect(() => {
    setSections(prevSections => {
      ...
      // List of all question types that have choices
      const choiceTypes = [
        'sq_ii_pr_i', 'sq_ii_pr_ii', 'mr_chips', 'essay', 'e_to_u', 'u_to_e', 'idioms'
      ];
  
      choiceTypes.forEach(type => {
        if (updatedSections[type]?.length) {
          const choiceValue = paperSettings[`${type}Choice`] || 0;
          updatedSections[type] = updatedSections[type].map(section => ({
            ...section,
            choice: choiceValue,
            required: Math.max(1, section.questions.length - choiceValue)
          }));
        }
      });
    ...
    paperSettings.sq_ii_pr_iChoice,
    paperSettings.sq_ii_pr_iiChoice,
    paperSettings.mr_chipsChoice,
    paperSettings.essayChoice,
    paperSettings.e_to_uChoice,
    paperSettings.u_to_eChoice,
    paperSettings.idiomsChoice
  ]);
      ...
      if (savedSettings) {
        try {
          ...
          setPaperSettings((prev) => ({
            ...prev,
            ...parsedSettings,
            subject: "English", // Ensure subject is set to English
          }));
  ...
  const preserveLineBreaks = (text, type) => {
    let lineHeight = paperSettings.lineHeight || "1.5"; // default from paper settings
    const marginBottom = "0"; // default margin

    // Get the appropriate line height based on question type
    switch (type) {
      case "mcqs":
        lineHeight = paperSettings.mcqsLineHeight;
        break;
      case "sq_ii_pr_i":
      case "sq_ii_pr_ii":
      case "mr_chips":
        lineHeight = paperSettings.shortLineHeight;
        break;
      case "e_to_u":
        lineHeight = paperSettings.e_to_uLineHeight;
        break;
      case "u_to_e":
        lineHeight = paperSettings.u_to_eLineHeight;
        break;
      case "essay":
  ...
  const totalMarks = useMemo(() => {
    ...
    Object.entries(sections).forEach(([type, typeSections]) => {
      typeSections.forEach((section) => {
        ...
        // Calculate required questions (ensure at least 1 question is required)
        const requiredQuestions = Math.max(1, questionsCount - choiceValue);
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const params = new URLSearchParams({
          class: "12th",
          subject: "English", // Use English for the fetch
        });
        ...
        const filteredQuestions = questions.filter((question) => {
          ...
          if (!topicMatch) {
            console.log(`Initial fetch: Question ${question.id} topic mismatch: ${question.topic} not in selected topics`);
          }
          return topicMatch;
        });
  ...
  const getTextStyle = (type = "normal") => {
    switch (type) {
      ...
      case "heading":
        return {
          fontFamily: paperSettings.headingFontStyle === "Default" ? "system-ui" : paperSettings.headingFontStyle,
  ...
  const handleAddSelection = () => {
    ...
    setSections((prevSections) => {
      ...
      Object.entries(questionsByType).forEach(([type, questions]) => {
        ...
        switch (type) {
          ...
          case "sq_ii_pr_ii":
          case "mr_chips":
            choiceValue = Math.min(paperSettings[`${type}Choice`] || 0, questions.length - 1);
            break;
          case "essay":
            choiceValue = Math.min(paperSettings.essayChoice || 0, questions.length - 1);
            break;
          case "e_to_u":
            choiceValue = Math.min(paperSettings.e_to_uChoice || 0, questions.length - 1);
            break;
          case "u_to_e":
            choiceValue = Math.min(paperSettings.u_to_eChoice || 0, questions.length - 1);
            break;
          case "idioms":
            choiceValue = Math.min(paperSettings.idiomsChoice || 0, questions.length - 1);
            break;
          default:
            choiceValue = 0;
        }
        ...
        // Set title based on question type with choice information
        let title = "";
        switch (type) {
          case "mcqs":
            title = "Choose the correct option";
          ...
          case "sq_ii_pr_i":
            title = choiceValue > 0
              ? `Write short answers to any ${requiredQuestions} of the following ${questions.length} questions (Book II Modern Prose & Heroes Part-I).`
              : "Write short answers to the following questions (Book II Modern Prose & Heroes Part-I).";
            break;
          case "sq_ii_pr_ii":
            title = choiceValue > 0
              ? `Write short answers to any ${requiredQuestions} of the following ${questions.length} questions (Book II Modern Prose & Heroes Part-II).`
              : "Write short answers to the following questions (Book II Modern Prose & Heroes Part-II).";
            break;
          case "mr_chips":
            title = choiceValue > 0
              ? `Write short answers to any ${requiredQuestions} of the following ${questions.length} questions (Mr. Chips).`
              : "Write short answers to the following questions (Mr. Chips).";
            break;
          case "e_to_u":
            title = "Translate the following paragraph into Urdu.";
            break;
    ...
    case "essay":
      title = choiceValue > 0
        ? `Write an essay on any ${requiredQuestions} of the following ${questions.length} topics.`
  ...
  const handlePrintClick = () => {
    ...
    const filename = `12th-English-${paperSettings.section || "section"}-${paperSettings.paperNumber || "paperNumber"}-${formattedDate}`
      .replace(/\s+/g, "-")
      .toLowerCase();
...
@media print {
  ...
  * {
  ...
  /* Ensure MCQ options are properly aligned */
  .grid.grid-cols-4,
  .grid.grid-cols-2,
...
`);
  ...
  function getHeaderPositionClasses() {
    ...
    return `${positionClasses[paperSettings.headerPosition] || "justify-start"} ${
  ...
  const groupQuestionsByType = (questions) => {
    // First, group questions by their type (mcqs, short, essay, etc.)
    const grouped = questions.reduce(
      ...
      { mcqs: [], sq_ii_pr_i: [], sq_ii_pr_ii: [], mr_chips: [], essay: [], e_to_u: [], u_to_e: [], idioms: [] }
    );
  ...
  // Add this single useEffect to handle ALL choice updates

// Update the getHeadingTextByType function to use the section's choice value
  const getHeadingTextByType = (type, section) => {
    const questions = section.questions;
    const totalQuestions = questions.length;
    const choiceValue = section.choice || 0;
    const marksPerQuestion = getMarksPerQuestion(type);
    const requiredQuestions = Math.max(1, totalQuestions - choiceValue);
    const totalMarks = requiredQuestions * marksPerQuestion;

    const choiceTexts = {
      mcqs: `Choose the correct option (${totalQuestions}×1=${totalQuestions})`,
      sq_ii_pr_i: choiceValue > 0 
        ? `Write short answers to the following questions (Book II Modern Prose & Heroes Part-I) (Any ${requiredQuestions} of ${totalQuestions}) (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`
        : `Write short answers to the following questions (Book II Modern Prose & Heroes Part-I) (${totalQuestions}×${marksPerQuestion}=${totalMarks})`,
      sq_ii_pr_ii: choiceValue > 0 
        ? `Write short answers to the following questions (Book II Modern Prose & Heroes Part-II) (Any ${requiredQuestions} of ${totalQuestions}) (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`
        : `Write short answers to the following questions (Book II Modern Prose & Heroes Part-II) (${totalQuestions}×${marksPerQuestion}=${totalMarks})`,
      mr_chips: choiceValue > 0 
        ? `Write short answers to the following questions (Mr. Chips) (Any ${requiredQuestions} of ${totalQuestions}) (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`
        : `Write short answers to the following questions (Mr. Chips) (${totalQuestions}×${marksPerQuestion}=${totalMarks})`,
      e_to_u: `Translate the following paragraph into Urdu. (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`,
      u_to_e: `Translate the following paragraph into English. (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`,
      essay: choiceValue > 0
        ? `Write an essay on any one of the following topics. (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`
        : `Write an essay on the following topic. (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`,
      idioms: `Use the following Idioms/Phrasal verbs in your own sentences. (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`
    };

    return section.title || choiceTexts[type] || "";
  };

  const calculateMarksForQuestionType = (questions, type) => {
    const totalQuestions = questions.length;
    if (totalQuestions === 0) return 0; // Return 0 if no questions
    ...
    switch (type) {
      case "sq_ii_pr_i":
        choice = Math.min(paperSettings.sq_ii_pr_iChoice || 0, totalQuestions - 1);
        break;
      case "sq_ii_pr_ii":
        choice = Math.min(paperSettings.sq_ii_pr_iiChoice || 0, totalQuestions - 1);
        break;
      case "mr_chips":
        choice = Math.min(paperSettings.mr_chipsChoice || 0, totalQuestions - 1);
        break;
      case "essay":
        choice = Math.min(paperSettings.essayChoice || 0, totalQuestions - 1);
        break;
      case "e_to_u":
        choice = Math.min(paperSettings.e_to_uChoice || 0, totalQuestions - 1);
        break;
      case "u_to_e":
        choice = Math.min(paperSettings.u_to_eChoice || 0, totalQuestions - 1);
        break;
      case "idioms":
        choice = Math.min(paperSettings.idiomsChoice || 0, totalQuestions - 1);
        break;
      default:
        choice = 0; // No choice for other types
  }
  
    // Calculate marks based on choice
    const questionsToAttempt = choice > 0 ? totalQuestions - choice : totalQuestions;
    return Math.max(0, questionsToAttempt * marksPerQuestion); // Ensure non-negative
  };
  ...
  const calculateTotalMarks = () => {
    ...
    const questionTypes = ["mcqs", "sq_ii_pr_i", "sq_ii_pr_ii", "mr_chips", "essay", "e_to_u", "u_to_e", "idioms"];
  
    questionTypes.forEach((type) => {
      const questions = selectedQuestions.filter((q) => q.type === type);
      totalMarks += calculateMarksForQuestionType(questions, type);
    });
        ...
        <div className="flex flex-col space-y-1 max-w-2xl">
          {options.map((option, idx) => (
            <div key={idx} className="flex items-center gap-2">
              <span className="font-medium w-6 text-left">({String.fromCharCode(65 + idx)})</span>
  ...
  const renderQuestionParts = (question) => {
    ...
    const partsMargin = paperSettings.longQuestionLineHeight > 0 ? "mt-2" : "mt-0";
      ...
      <div className={`ml-6 ${partsMargin}`}>
        {question.parts.map((part, idx) => (
          ...
          >
            ...
            >
              <span className="font-medium mr-3 inline-block">{idx === 0 ? "a)" : idx === 1 ? "b)" : idx === 2 ? "c)" : "d)"}</span>
            </div>
            <div className="flex-1" style={{ lineHeight: "inherit", display: "block" }}>
              <span
  ...
  return (
    <ProtectedRoute>
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100" style={paperContainerStyle}>
        {/* Navigation Bar */}
            ...
            <div className="bg-white rounded-xl w-[98%] max-w-[1920px] shadow-2xl border border-violet-100">
              ...
              <div className="bg-gradient-to-r from-violet-600 to-indigo-500 text-white px-4 py-2.5 flex justify-between items-center rounded-t-xl">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-base bg-white/20 px-3 py-0.5 rounded-full">12th - English</span>
                </div>
                <div className="flex items-center gap-3">
                  <button
                  ...
                  <div className="grid grid-cols-3 gap-6">
                    <div className="flex items-center gap-3">
                      ...
                      >
                        ...
                        <option value="sq_ii_pr_i">Short Questions (Book II Part-I)</option>
                        <option value="sq_ii_pr_ii">Short Questions (Book II Part-II)</option>
                        <option value="mr_chips">Short Questions (Mr. Chips)</option>
                        <option value="e_to_u">English to Urdu Translation</option>
                        <option value="u_to_e">Urdu to English Translation</option>
                        <option value="essay">Essay</option>
                        <option value="idioms">Idioms/Phrasal Verbs</option>
                      </select>
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-4">
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          type="number"
                          min="0"
                          value={paperSettings.sq_ii_pr_iChoice || 0}
                          onChange={(e) =>
                            setPaperSettings((prev) => ({ ...prev, sq_ii_pr_iChoice: Number.parseInt(e.target.value) || 0 }))
                          }
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          type="number"
                          min="0"
                          value={paperSettings.sq_ii_pr_iiChoice || 0}
                          onChange={(e) =>
                            setPaperSettings((prev) => ({ ...prev, sq_ii_pr_iiChoice: Number.parseInt(e.target.value) || 0 }))
                          }
                          className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
                        />
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          type="number"
                          min="0"
                          value={paperSettings.mr_chipsChoice || 0}
                          onChange={(e) =>
                            setPaperSettings((prev) => ({ ...prev, mr_chipsChoice: Number.parseInt(e.target.value) || 0 }))
                          }
                          className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
                        />
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          type="number"
                          min="0"
                          value={paperSettings.e_to_uChoice || 0}
                          onChange={(e) =>
                            setPaperSettings((prev) => ({ ...prev, e_to_uChoice: Number.parseInt(e.target.value) || 0 }))
                          }
                          className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-4 gap-4 mt-2">
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          type="number"
                          min="0"
                          value={paperSettings.u_to_eChoice || 0}
                          onChange={(e) =>
                            setPaperSettings((prev) => ({ ...prev, u_to_eChoice: Number.parseInt(e.target.value) || 0 }))
                          }
                          className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
                        />
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          onChange={(e) => handleEssayChoiceChange(e.target.value)}
                          className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
                        />
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          type="number"
                          min="0"
                          value={paperSettings.idiomsChoice || 0}
                          onChange={(e) =>
                            setPaperSettings((prev) => ({ ...prev, idiomsChoice: Number.parseInt(e.target.value) || 0 }))
                          }
                          className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
                        />
                      </div>
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <span className="px-2 py-1 bg-violet-50 text-violet-600 rounded text-xs">
                                Marks: {question.marks || getMarksPerQuestion(question.type)}
                              </span>
                            </div>
                            <p
                            ...
                            {question.images?.length > 0 && (
                              <div
                                className={`my-4 ${
                                  paperSettings.imageSettings.position === "left"
                                    ? "text-left"
                                    : paperSettings.imageSettings?.position === "right"
                                    ? "text-right"
                                    : "text-center"
                                }`}
                                style={{
                ...
                <div className="space-y-3">
                  ...
                  <div className="mt-6 space-y-4 border-t border-gray-200 pt-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs text-gray-600 mb-1">MCQ Line Height</label>
                        <input
                          type="number"
                          step="0.1"
                          value={paperSettings.mcqsLineHeight}
                          onChange={(e) =>
                            setPaperSettings((prev) => ({
                              ...prev,
                              mcqsLineHeight: e.target.value,
                            }))
                          }
                          className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30"
                        />
                      ...
                      <div>
                        <label className="block text-xs text-gray-600 mb-1">Short Question Line Height</label>
                        <input
                          type="number"
                          step="0.1"
                          value={paperSettings.shortLineHeight}
                          onChange={(e) =>
                            setPaperSettings((prev) => ({
                              ...prev,
                              shortLineHeight: e.target.value,
                            }))
                          }
                          className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30"
                        />
                      ...
                      <div>
                        <label className="block text-xs text-gray-600 mb-1">English to Urdu Line Height</label>
                        <input
                          type="number"
                          step="0.1"
                          value={paperSettings.e_to_uLineHeight}
                          onChange={(e) =>
                            setPaperSettings((prev) => ({
                              ...prev,
                      ...
                      <div>
                        <label className="block text-xs text-gray-600 mb-1">Urdu to English Line Height</label>
                        <input
                          type="number"
                          step="0.1"
                          value={paperSettings.u_to_eLineHeight}
                          onChange={(e) =>
                            setPaperSettings((prev) => ({
                              ...prev,
                      ...
                      <div>
                        <label className="block text-xs text-gray-600 mb-1">Essay Line Height</label>
                        <input
                          type="number"
                          step="0.1"
                      ...
                      <div>
                        <label className="block text-xs text-gray-600 mb-1">Idioms Line Height</label>
                        <input
                          type="number"
                          step="0.1"
