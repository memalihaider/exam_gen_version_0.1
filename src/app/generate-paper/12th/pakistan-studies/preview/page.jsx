...
export default function PaperPreview() {
  ...
  const [sections, setSections] = useState({
    mcqs: [{ title: "درست جواب کا انتخاب کریں۔", questions: [], choice: 0 }],
    short: [{ title: "مختصر سوالات", questions: [], choice: 0 }],
    long: [{ title: "تفصیلی سوالات", questions: [], choice: 0 }],
    q_w_m: [{ title: " قرآنی الفاظ کے معنٰی ", questions: [], choice: 0 }],
    ayat: [{ title: "آیات کا ترجمہ", questions: [], choice: 0 }],
    note: [{ title: "نوٹ", questions: [], choice: 0 }],
  });
  ...
  useEffect(() => {
    ...
    const savedExercises = localStorage.getItem("selectedPakStudies12Exercises");
    if (savedExercises) {
      try {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const subject = paperSettings.subject || "Pakistan Studies"; // Get current subject
      ...
      if (savedSettings) {
        try {
          ...
          setPaperSettings((prev) => ({
            ...prev,
            ...parsedSettings,
            subject: "Pakistan Studies", // Ensure subject is set to Islamiat
          }));
  ...
  const getMarksPerQuestion = (type) => {
    switch (type) {
      ...
      case "long":
        return 4; // Long/detailed questions - 4 marks
      case "q_w_m":
        return 1; // Hadith translation - 3 marks
      case "ayat":
        return 3; // Hadith translation - 3 marks
      case "note":
        return 5; // Note - 5 marks
      default:
        return 0;
    }
  };
...
const preserveLineBreaks = (text, type) => {
  let lineHeight = "1.5"; // default line height
  ...
  switch (type) {
    ...
    case "ayat":
      ...
      marginBottom = "0.15 rem"; // Add spacing after ayat questions
      break;
    case "note":
      lineHeight = paperSettings.noteLineHeight;
      marginBottom = "0.15 rem"; // Add spacing after note questions
      break;
    default:
      lineHeight = "1.5";
  }
  ...
  return {
    ...
    marginBottom: marginBottom // Add the margin to create spacing
  };
};
...
// ... existing code ...
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const params = new URLSearchParams({
          class: "12th",
          subject: "Pakistan Studies", // Use Islamiat for the fetch
        });
    ...
    setSections((prevSections) => {
      ...
      Object.entries(questionsByType).forEach(([type, questions]) => {
        ...
        switch (type) {
          ...
          case "q_w_m":
            choiceValue = Math.min(
              paperSettings.q_w_mChoice || 0,
              questions.length - 1
            );
            break;
          case "ayat":
            choiceValue = Math.min(
              paperSettings.ayatChoice || 0,
              questions.length - 1
            );
            break;
          case "note":
            choiceValue = Math.min(
              paperSettings.noteChoice || 0,
              questions.length - 1
            );
            break;
          default:
        ...
        switch (type) {
          ...
          case "long":
            title =
              choiceValue > 0
                ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} تفصیلی سوالات کے جوابات تحریر کریں۔`
                : "تفصیلی سوالات";
            break;
          case "q_w_m":
            title =
              choiceValue > 0
                ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} احادیث کا ترجمہ کریں۔`
                : "احادیث کا ترجمہ";
            break;
          case "ayat":
            title =
              choiceValue > 0
                ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} آیات کا ترجمہ کریں۔`
                : "آیات کا ترجمہ";
            break;
          case "note":
            title =
              choiceValue > 0
                ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} نوٹ تحریر کریں۔`
                : "نوٹ";
            break;
          default:
  ...
  const handlePrintClick = () => {
    ...
    const filename = `12th-Pakistan-Studies-${paperSettings.section || "section"}-${
...
`);
  ...
  const handleSavePaper = async () => {
    try {
      ...
      const paperData = {
        ...
        subject: paperSettings.subject || "Pakistan Studies",
  ...
  const getHeadingTextByType = (type, section) => {
    ...
    let choiceValue = 0;
    let marksPerQuestion = getMarksPerQuestion(type);
    ...
    switch (type) {
      case "short":
        ...
        break;
      case "long":
        choiceValue = paperSettings.longChoice || 0;
        break;
      case "q_w_m":
        choiceValue = paperSettings.q_w_mChoice || 0;
        break;
      case "ayat":
        choiceValue = paperSettings.ayatChoice || 0;
        break;
      case "note":
        choiceValue = paperSettings.noteChoice || 0;
        break;
    }

    const requiredQuestions =
      choiceValue > 0 ? totalQuestions - choiceValue : totalQuestions;
    const totalMarks = requiredQuestions * marksPerQuestion;

    switch (type) {
      case "mcqs":
        return `درست جواب کا انتخاب کریں۔ (${totalQuestions}×1=${totalQuestions})`;
      case "short":
        return `مختصر سوالات (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      case "long":
        return `تفصیلی سوالات (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      case "q_w_m":
        return ` قرآنی الفاظ کے معنٰی لکھیں۔ ${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      case "ayat":
        return `آیت کا ترجمہ (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      case "note":
        return `نوٹ (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      default:
        return "";
    }
  };
  ...
  const calculateMarksForQuestionType = (questions, type) => {
    ...
    switch (type) {
      case "short":
        ...
        break;
      case "long":
        choice = paperSettings.longChoice || 0;
        break;
      case "q_w_m":
        choice = paperSettings.q_w_mChoice || 0;
        break;
      case "ayat":
        choice = paperSettings.ayatChoice || 0;
        break;
      case "note":
        choice = paperSettings.noteChoice || 0;
        break;
    }

    // Calculate marks based on choice
    if (["short", "long", "q_w_m", "ayat", "note"].includes(type)) {
  ...
  const calculateTotalMarks = () => {
    ...
    const calculateTypeMarks = (type, choice) => {
      ...
      const required =
        choice > 0 ? questions.length - choice : questions.length;
    ...
    const longMarks = calculateTypeMarks("long", paperSettings.longChoice);
    const q_w_mMarks = calculateTypeMarks("q_w_m", paperSettings.q_w_mChoice);
    const ayatMarks = calculateTypeMarks("ayat", paperSettings.ayatChoice);
    const noteMarks = calculateTypeMarks("note", paperSettings.noteChoice);

    return (
      mcqsMarks + shortMarks + longMarks + q_w_mMarks + ayatMarks + noteMarks
    );
  };
  ...
  const calculatePaperTotalMarks = () => {
    ...
    const questionTypes = ["mcqs", "short", "long", "q_w_m", "ayat", "note"];
    ...
    <ProtectedRoute>
            ...
            >
              ...
              <div className="p-4 bg-gradient-to-br from-gray-50 to-white">
                ...
                <div className="space-y-4 bg-white p-6 rounded-xl shadow-sm border border-violet-100">
                  ...
                  <div className="grid grid-cols-3 gap-6">
                    <div className="flex items-center gap-3">
                      ...
                      >
                        ...
                        <option value="mcqs">MCQs (ایم سی کیوز)</option>
                        <option value="short">
                          Short Questions (مختصر سوالات)
                        </option>
                        <option value="long">
                          Long Questions (تفصیلی سوالات)
                        </option>
                        {/* <option value="q_w_m">
                          Words Meanings (قرآنی الفاظ کے معنٰی)
                        </option>
                        <option value="ayat">
                          Ayat Translation (آیات کا ترجمہ)
                        </option>
                        <option value="note">Note (نوٹ)</option> */}
                      </select>
                    </div>
                  ...
                  {/* Row 2: Medium, Short Choice, Long Choice */}
                  <div className="grid grid-cols-3 gap-6">
                    {/* <div className="flex items-center gap-3">
      <div className="bg-gradient-to-r from-violet-600 to-indigo-500 text-white px-4 py-2.5 text-sm rounded-lg font-medium shadow-sm whitespace-nowrap">
        Medium
      ...
      >
        ...
        <option value="URDU MEDIUM">URDU MEDIUM</option>
                  ...
                  {/* Row 3: Ayat Choice, Hadith Choice, Note Choice */}
                  <div className="grid grid-cols-3 gap-6">
                    {/* <div className="flex items-center gap-3">
                      <div className="bg-gradient-to-r from-violet-600 to-indigo-500 text-white px-4 py-2.5 text-sm rounded-lg font-medium shadow-sm whitespace-nowrap">
                        Ayat Choice
                      ...
                      <input
                        type="number"
                        min="0"
                        value={paperSettings.ayatChoice || 0}
                        onChange={(e) =>
                          setPaperSettings((prev) => ({
                            ...prev,
                            ayatChoice: parseInt(e.target.value) || 0,
                          }))
                        }
                        className="w-full border border-violet-100 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
                      />
                    ...
                    {/* <div className="flex items-center gap-3">
                      ...
                      <input
                        type="number"
                        min="0"
                        value={paperSettings.q_w_mChoice || 0}
                        onChange={(e) =>
                          setPaperSettings((prev) => ({
                            ...prev,
                            q_w_mChoice: parseInt(e.target.value) || 0,
                          }))
                        }
                        className="w-full border border-violet-100 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
                      />
                    ...
                    {/* <div className="flex items-center gap-3">
                      ...
                      <input
                        type="number"
                        min="0"
                        value={paperSettings.noteChoice || 0}
                        onChange={(e) =>
                          setPaperSettings((prev) => ({
                            ...prev,
                            noteChoice: parseInt(e.target.value) || 0,
                          }))
                        }
                        className="w-full border border-violet-100 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
                      />
            ...
            <div className="p-6 space-y-6" dir="ltr">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  {/* <div className="mt-3">
                    <label className="block text-sm text-gray-600 mb-1">
                      Words Meaning Line Height
                    </label>
                    <input
                      type="number"
                      step="0.1"
                      value={paperSettings.q_w_mLineHeight}
                      onChange={(e) =>
                        setPaperSettings((prev) => ({
                          ...prev,
                          q_w_mLineHeight: e.target.value,
                        }))
                      }
                      className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Affects words meaning text spacing
                    </p>
                  </div> */}

                  {/* <div className="mt-3">
                    <label className="block text-sm text-gray-600 mb-1">
                      Ayat Translation Line Height
                    </label>
                    <input
                      type="number"
                      step="0.1"
                      value={paperSettings.ayatLineHeight}
                      onChange={(e) =>
                        setPaperSettings((prev) => ({
                          ...prev,
                          ayatLineHeight: e.target.value,
                        }))
                      }
                      className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Affects ayat translation text spacing
                    </p>
                  </div> */}
                  ...
                  {/* <div className="mt-3">
                    ...
                    <input
                      type="number"
                      step="0.1"
                      value={paperSettings.noteLineHeight}
                      onChange={(e) =>
                        setPaperSettings((prev) => ({
                          ...prev,
                          noteLineHeight: e.target.value,
                        }))
                      }
                      className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Affects note text spacing
                    </p>
                  </div> */}
                </div>
              </div>
