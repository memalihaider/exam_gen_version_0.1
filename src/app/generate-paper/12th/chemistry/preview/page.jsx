...
// import { ninthChemistry} from "@/data/dummyQuestions"
...
export default function PaperPreview() {
  ...
  useEffect(() => {
    ...
    const savedExercises = localStorage.getItem("selectedChemistry12Exercises");
    if (savedExercises) {
      try {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const subject = paperSettings.subject || "Chemistry"; // Get current subject
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const params = new URLSearchParams({
          class: "12th",
          subject: "Chemistry", // Always use Chemistry for the initial fetch
        });
  ...
  const handleSearchQuestions = () => {
    ...
    // Build query parameters - IMPORTANT: Always use Chemistry as the subject since that's what the topics are for
    const params = new URLSearchParams({
      class: "12th",
      subject: "Chemistry", // Hardcode to Chemistry since the topics are from Chemistry
    });
  ...
  const handlePrintClick = () => {
    ...
    const filename = `12th-Chemistry-${paperSettings.section || "section"}-${
  ...
  const handleSavePaper = async () => {
    try {
      ...
      const paperData = {
        ...
        subject: paperSettings.subject || "Chemistry",
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td className="border border-gray-200 p-2 w-1/4 bg-gray-50">
                            ...
                            <span className="text-gray-700 text-sm">
                              {paperSettings.subject || "Chemistry"}
                            </span>
                          </td>
