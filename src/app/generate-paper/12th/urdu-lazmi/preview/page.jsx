...
const [sections, setSections] = useState({
  ...
  poet_ex: [{ title: "اشعار کی تشریح حصہ نطم", questions: [], choice: 0 }],
  ...
  essay: [{ title: "مضمون", questions: [], choice: 0 }],
...
// ... existing code ...
  ...
  // Add this function after the existing handleShortChoiceChange and handleLongChoiceChange functions (around line 350)

const handleChoiceChange = (type, value) => {
  setPaperSettings((prev) => ({
    ...prev,
    [`${type}Choice`]: Math.max(0, Number.parseInt(value) || 0),
  }));
  
  // Update the sections state to reflect the new choice value
  setSections((prevSections) => {
    const newSections = { ...prevSections };
    if (newSections[type]) {
      newSections[type] = newSections[type].map(section => ({
        ...section,
        choice: Math.max(0, Number.parseInt(value) || 0)
      }));
    }
    return newSections;
  });
  ...
  useEffect(() => {
    ...
    const savedExercises = localStorage.getItem("selectedUrduLazmi12Exercises");
    if (savedExercises) {
      try {
  ...
  // Update sections with paperSettings choice values after settings are loaded
useEffect(() => {
  setSections(prevSections => {
    const newSections = { ...prevSections };
    
    // Update each section type with the corresponding choice value from paperSettings
    Object.keys(newSections).forEach(type => {
      if (newSections[type] && newSections[type].length > 0) {
        const choiceValue = paperSettings[`${type}Choice`] || 0;
        newSections[type] = newSections[type].map(section => ({
          ...section,
          choice: choiceValue
        }));
...
}, [paperSettings.mcqsChoice, paperSettings.poet_exChoice, paperSettings.ghazl_exChoice, 
    paperSettings.proseExpChoice, paperSettings.poem_summChoice, paperSettings.lessonSummChoice, 
    paperSettings.essayChoice, paperSettings.letterChoice]);
  ...
  const getMarksPerQuestion = (type) => {
    switch (type) {
      ...
      case "poet_ex":
        return 4; // Poetry explanation (Nazm) - 4 marks
      case "ghazl_ex":
        return 3; // Poetry explanation (Ghazal) - 3 marks
      case "proseExp":
        return 10; // Prose explanation - 10 marks
      case "poem_summ":
        return 5; // Poem summary - 5 marks
      case "lessonSumm":
        return 10; // Lesson summary - 10 marks
      case "essay":
        return 20; // Essay - 20 marks
      case "letter":
        return 10; // Letter - 10 marks
      default:
        return 0;
    }
  };
...
const preserveLineBreaks = (text, type) => {
  ...
  switch (type) {
    ...
    case "short":
      ...
      break;
    case "essay":
      lineHeight = paperSettings.essayLineHeight || paperSettings.lineHeight;
      marginBottom = "0.3rem";
      break;
    case "letter":
      ...
      break;
    case "comprehense":
      lineHeight = paperSettings.comprehenseLineHeight || paperSettings.lineHeight;
      marginBottom = "0.3rem";
      break;
    default:
      lineHeight = paperSettings.lineHeight;
  ...
  return {
    ...
    display: "inline", // Add this to ensure proper text flow
    verticalAlign: "top" // Add this to align with numbers
  };
};
// ... existing code ...
...
const totalMarks = useMemo(() => {
  ...
  Object.entries(sections).forEach(([type, typeSections]) => {
    typeSections.forEach((section) => {
      ...
      // Get the choice value directly from the section
      const choiceValue = section.choice || 0;
      
      // Calculate required questions (total - choice)
      const requiredQuestions = Math.max(0, questionsCount - choiceValue);
      
      // Calculate marks for this section
      marks += requiredQuestions * marksPerQuestion;
    });
  });
...
// ... existing code ...
    ...
    setSections((prevSections) => {
      ...
      Object.entries(questionsByType).forEach(([type, questions]) => {
        ...
        switch (type) {
          ...
          case "essay":
            choiceValue = Math.min(
              paperSettings.essayChoice || 0,
              questions.length - 1
            );
            break;
          case "letter":
            choiceValue = Math.min(
              paperSettings.letterChoice || 0,
              questions.length - 1
            );
            break;
          default:
        ...
        switch (type) {
          case "mcqs":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} درست جواب کا انتخاب کریں۔`
              : "درست جواب کا انتخاب کریں۔";
            break;
          case "poet_ex":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} نظم کے اشعار کی تشریح کیجیے۔نطم کا عنوان اور شاعر کا نام بھی تحریر کیجیے۔`
              : "مندرجہ ذیل نظم کے اشعار کی تشریح کیجیے۔نطم کا عنوان اور شاعر کا نام بھی تحریر کیجیے۔";
            break;
          case "ghazl_ex":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} غزل کے اشعار کی تشریح کیجیے۔ شاعر کا نام بھی لکھیے۔`
              : "مندرجہ ذیل غزل کے اشعار کی تشریح کیجیے۔ شاعر کا نام بھی لکھیے۔";
            break;
          case "proseExp":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} نثر پاروں کی تشریح کیجیے۔سبق کاعنوان، مصنف کا نام اور سیاق و سباق بھی تحریر کیجیے۔`
              : "درج ذیل نثر پاروں کی تشریح کیجیے۔سبق کاعنوان، مصنف کا نام اور سیاق و سباق بھی تحریر کیجیے۔";
            break;
          case "poem_summ":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} نظم کا خلاصہ تحریر کیجیے۔اور مصنف کا نام بھی تحریر کیجیے۔`
              : "درج ذیل نظم کا خلاصہ تحریر کیجیے۔اور مصنف کا نام بھی تحریر کیجیے۔";
            break;
          case "lessonSumm":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} سبق کا خلاصہ لکھیے اور مصنف کا نام بھی تحریر کیجیے۔`
              : "درج ذیل سبق کا خلاصہ لکھیے اور مصنف کا نام بھی تحریر کیجیے۔";
            break;
          case "essay":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} عنوان پر مضمون تحریر کیجیے۔`
              : "درج ذیل عنوان پر مضمون تحریر کیجیے۔";
            break;
          case "letter":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} خط تحریر کیجیے۔`
              : "خط تحریر کیجیے۔";
            break;
          default:
            title = `درج ذیل ${questions.length} میں سے ${requiredQuestions} سوالات کے جوابات دیں۔`;
        }
        ...
        const newSection = {
          ...
          choice: (() => {
            switch (type) {
              case "mcqs": return paperSettings.mcqsChoice || 0;
              case "poet_ex": return paperSettings.poet_exChoice || 0;
              case "ghazl_ex": return paperSettings.ghazl_exChoice || 0;
              case "proseExp": return paperSettings.proseExpChoice || 0;
              case "poem_summ": return paperSettings.poem_summChoice || 0;
              case "lessonSumm": return paperSettings.lessonSummChoice || 0;
              case "essay": return paperSettings.essayChoice || 0;
              case "letter": return paperSettings.letterChoice || 0;
              default: return 0;
            }
          })(),
          required: Math.max(1, questions.length - ((() => {
            switch (type) {
              case "mcqs": return paperSettings.mcqsChoice || 0;
              case "poet_ex": return paperSettings.poet_exChoice || 0;
              case "ghazl_ex": return paperSettings.ghazl_exChoice || 0;
  ...
  const handlePrintClick = () => {
    ...
    const filename = `12th-Urdu-${paperSettings.section || "section"}-${
...
`);
  ...
  const handleSavePaper = async () => {
    try {
      ...
      const paperData = {
        class: paperSettings.class || "12th",
  ...
  switch (type) {
    case "mcqs":
      return choiceValue > 0 
        ? `ایم سی کیوز (${requiredQuestions}×1=${totalMarks}) - ${totalQuestions} میں سے ${requiredQuestions} کا انتخاب کریں`
        : `ایم سی کیوز (${totalQuestions}×1=${totalMarks})`;
    case "poet_ex":
      return choiceValue > 0 
        ? `اشعار کی تشریح حصہ نطم (${requiredQuestions}×4=${totalMarks}) - ${totalQuestions} میں سے ${requiredQuestions} کا انتخاب کریں`
        : `اشعار کی تشریح حصہ نطم (${requiredQuestions}×4=${totalMarks})`;
    case "ghazl_ex":
      return choiceValue > 0 
        ? `اشعار کی تشریح حصہ غزل (${requiredQuestions}×3=${totalMarks}) - ${totalQuestions} میں سے ${requiredQuestions} کا انتخاب کریں`
        : `اشعار کی تشریح حصہ غزل (${requiredQuestions}×3=${totalMarks})`;
    case "proseExp":
      return choiceValue > 0 
        ? `نثر پاروں کی تشریح (${requiredQuestions}×10=${totalMarks}) - ${totalQuestions} میں سے ${requiredQuestions} کا انتخاب کریں`
        : `نثر پاروں کی تشریح (${requiredQuestions}×10=${totalMarks})`;
    case "poem_summ":
      return choiceValue > 0 
        ? `نظم کا خلاصہ (${requiredQuestions}×5=${totalMarks}) - ${totalQuestions} میں سے ${requiredQuestions} کا انتخاب کریں`
        : `نظم کا خلاصہ (${requiredQuestions}×5=${totalMarks})`;
    case "lessonSumm":
      return choiceValue > 0 
        ? `سبق کا خلاصہ (${requiredQuestions}×10=${totalMarks}) - ${totalQuestions} میں سے ${requiredQuestions} کا انتخاب کریں`
        : `سبق کا خلاصہ (${requiredQuestions}×10=${totalMarks})`;
    case "essay":
      return choiceValue > 0 
        ? `مضمون (${requiredQuestions}×20=${totalMarks}) - ${totalQuestions} میں سے ${requiredQuestions} کا انتخاب کریں`
        : `مضمون (${requiredQuestions}×20=${totalMarks})`;
    case "letter":
      return choiceValue > 0 
        ? `خط (${requiredQuestions}×10=${totalMarks}) - ${totalQuestions} میں سے ${requiredQuestions} کا انتخاب کریں`
        : `خط (${requiredQuestions}×10=${totalMarks})`;
    default:
      return "";
  }
};
  ...
  const calculateMarksForQuestionType = (questions, type) => {
    ...
    switch (type) {
      case "mcqs":
        choice = paperSettings.mcqsChoice || 0;
        break;
      case "poet_ex":
      ...
      case "short":
        ...
        break;
      case "essay":
        choice = paperSettings.essayChoice || 0;
        break;
      case "comprehense":
        choice = paperSettings.comprehenseChoice || 0;
        break;
    case "letter":
    ...
    if (["mcqs", "poet_ex", "ghazl_ex", "proseExp", "poem_summ", "lessonSumm", "short", "essay", "comprehense", "letter"].includes(type)) {
      const questionsToAttempt =
        choice > 0 ? totalQuestions - choice : totalQuestions;
      return questionsToAttempt * marksPerQuestion;
    }

    // For other types, return all marks as there's no choice
...
const calculatePaperTotalMarks = () => {
  ...
  const questionTypes = [
    "mcqs", "poet_ex", "ghazl_ex", "proseExp", "poem_summ", "lessonSumm",
    "essay", "letter"
  ];

  questionTypes.forEach((type) => {
    // Get all questions of this type from sections
    const sectionQuestions = sections[type]?.[0]?.questions || [];
    const questionsCount = sectionQuestions.length;
    
    // Get choice value from paperSettings
    const choiceValue = paperSettings[`${type}Choice`] || 0;
    
    // Get marks per question for this type
    const marksPerQuestion = getMarksPerQuestion(type);
    
    // Calculate total marks for this type
    const typeMarks = (questionsCount - choiceValue) * marksPerQuestion;
  ...
  <div className="grid grid-cols-3 gap-6">
    <div className="flex items-center gap-3">
      ...
      >
        ...
        <option className="font-nastaliq" value="poet_ex">اشعار کی تشریح حصہ نطم</option>
        ...
        <option className="font-nastaliq" value="essay">مضمون</option>
        <option className="font-nastaliq" value="letter">خط</option>
        {/* <option className="font-nastaliq" value="short">مختصر سوالات</option> */}
        {/* <option className="font-nastaliq" value="comprehense">تفہیم عبارت</option> */}
      </select>
    </div>
  ...
  <div className="bg-gradient-to-r from-violet-600 to-indigo-500 text-white px-4 py-2 mb-3 text-center rounded-lg font-medium shadow-sm  text-xl">
    ...
    <div className="flex items-center gap-2">
      <div className="bg-violet-100 text-violet-700 px-3 py-2 text-xs rounded-lg font-medium shadow-sm whitespace-nowrap font-nastaliq">
        اشعار کی تشریح حصہ نطم
      </div>
      <input
        type="number"
        min="0"
        value={paperSettings.poet_exChoice || 0}
        onChange={(e) => handleChoiceChange('poet_ex', e.target.value)}
    ...
    <div className="flex items-center gap-2">
      ...
      <input
        ...
        onChange={(e) => handleChoiceChange('ghazl_ex', e.target.value)}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
    <div className="flex items-center gap-2">
      ...
      <input
        ...
        onChange={(e) => handleChoiceChange('proseExp', e.target.value)}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
    <div className="flex items-center gap-2">
      ...
      <input
        ...
        onChange={(e) => handleChoiceChange('poem_summ', e.target.value)}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
  </div>
  ...
  <div className="grid grid-cols-4 gap-4 mt-2">
    <div className="flex items-center gap-2">
      ...
      <input
        ...
        onChange={(e) => handleChoiceChange('lessonSumm', e.target.value)}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
    <div className="flex items-center gap-2">
      ...
      <input
        type="number"
        min="0"
        value={paperSettings.essayChoice || 0}
        onChange={(e) => handleChoiceChange('essay', e.target.value)}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
    <div className="flex items-center gap-2">
      ...
      <input
        ...
        onChange={(e) => handleChoiceChange('letter', e.target.value)}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
    <div className="flex items-center gap-2">
      ...
      <input
        type="number"
        min="0"
        value={paperSettings.mcqsChoice || 0}
        onChange={(e) => handleChoiceChange('mcqs', e.target.value)}
        className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
      />
    </div>
  </div>
...
<div className="mt-6 space-y-4 border-t border-gray-200 pt-4">
  ...
  <div className="grid grid-cols-2 gap-4">
    ...
    <div>
      <label className="block text-xs text-gray-600 mb-1">
        Poetry Explanation (Nazm) Line Height
      </label>
      <input
        type="number"
        step="0.1"
    ...
    <div>
      ...
      <input
        type="number"
        step="0.1"
        value={paperSettings.essayLineHeight || paperSettings.lineHeight}
        onChange={(e) =>
          setPaperSettings((prev) => ({
            ...prev,
            essayLineHeight: e.target.value,
          }))
        }
        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-violet-500/30 text-left"
      />
