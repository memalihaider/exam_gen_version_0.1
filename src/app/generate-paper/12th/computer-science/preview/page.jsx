...
// import { ninthComputerScience} from "@/data/dummyQuestions"
...
export default function PaperPreview() {
  ...
  useEffect(() => {
    ...
    const savedExercises = localStorage.getItem("selectedComputerScience12Exercises");
    if (savedExercises) {
      try {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const subject = paperSettings.subject || "Computer Science"; // Get current subject
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const params = new URLSearchParams({
          class: "12th",
          subject: "Computer Science", // Always use Computer Science for the initial fetch
        });
  ...
  const handleSearchQuestions = () => {
    ...
    // Build query parameters - IMPORTANT: Always use Computer Science as the subject since that's what the topics are for
    const params = new URLSearchParams({
      class: "12th",
      subject: "Computer Science", // Hardcode to Computer Science since the topics are from Computer Science
    });
  ...
  const handlePrintClick = () => {
    ...
    const filename = `12th-Computer-Science-${paperSettings.section || "section"}-${
  ...
  const handleSavePaper = async () => {
    try {
      ...
      const paperData = {
        ...
        subject: paperSettings.subject || "Computer Science",
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td className="border border-gray-200 p-2 w-1/4 bg-gray-50">
                            ...
                            <span className="text-gray-700 text-sm">
                              {paperSettings.subject || "Computer Science"}
                            </span>
                          </td>
