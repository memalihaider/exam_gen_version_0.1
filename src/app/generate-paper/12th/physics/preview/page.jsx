...
// import { ninthPhysics} from "@/data/dummyQuestions"
...
export default function PaperPreview() {
  ...
  useEffect(() => {
    ...
    const savedExercises = localStorage.getItem("selectedPhysics12Exercises");
    if (savedExercises) {
      try {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const subject = paperSettings.subject || "Physics"; // Get current subject
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const params = new URLSearchParams({
          class: "12th",
          subject: "Physics", // Always use Physics for the initial fetch
        });
  ...
  const handleAddSelection = () => {
    const selectedQuestions = searchedQuestions.filter((q) =>
      selectedQuestionIds.has(q.id)
    );

    // Group selected questions by type
    const questionsByType = selectedQuestions.reduce((acc, question) => {
    ...
    setSections((prevSections) => {
      ...
      Object.entries(questionsByType).forEach(([type, questions]) => {
        ...
        // Get the appropriate choice value for this type
        let choiceValue = 0;
        if (type === "short") {
        ...
        // Calculate the required number of questions to attempt
        const requiredQuestions = Math.max(1, questions.length - choiceValue);

        // Create a new section title based on type and requirements
        const title =
          type === "mcqs"
            ? "Choose the correct option."
            : `ATTEMPT ANY ${requiredQuestions} OF THE FOLLOWING ${questions.length} QUESTIONS`;

        // Create new section with unique identifier
        const newSection = {
        ...
        // Replace existing section if it exists, otherwise add new one
  ...
  const handleSearchQuestions = () => {
    ...
    // Build query parameters - IMPORTANT: Always use Physics as the subject since that's what the topics are for
    const params = new URLSearchParams({
      class: "12th",
      subject: "Physics", // Hardcode to Physics since the topics are from Physics
    });
 ...
 const filename = `12th-biology-${paperSettings.section || "section"}-${paperSettings.paperNumber || "paperNumber"}-${formattedDate}`
   .replace(/\s+/g, "-")
   .toLowerCase();
  ...
  const handleSavePaper = async () => {
    try {
      ...
      const paperData = {
        ...
        subject: paperSettings.subject || "Physics",
