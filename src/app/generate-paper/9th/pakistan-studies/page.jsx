...
import { pakistanStudiesUnits } from './unitsData';

const PakistanStudiesNinth = () => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-12 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-12 text-center">
          <h1 className="text-5xl font-bold mb-4 bg-gradient-to-r from-red-600 via-red-500 to-orange-500 bg-clip-text text-transparent">
            Pakistan Studies - 9th Class
          </h1>
          <p className="text-gray-600 mb-6">Select topics to generate a custom paper</p>
          <div className="flex items-center justify-center gap-2 text-sm">
            <Link href="/" className="text-red-600 hover:text-red-800 transition-colors">Home</Link>
            <span className="text-gray-500">/</span>
            <Link href="/generate-paper" className="text-red-600 hover:text-red-800 transition-colors">Generate Paper</Link>
            <span className="text-gray-500">/</span>
            <Link href="/generate-paper/9th" className="text-red-600 hover:text-red-800 transition-colors">9th Class</Link>
            <span className="text-gray-500">/</span>
            ...
            >
                ...
                <label className="flex items-center justify-between cursor-pointer group">
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      className="w-6 h-6 text-red-600 rounded-lg border-gray-300 focus:ring-red-500 transition-all"
                    />
                  </div>
                </label>
              </div>
              ...
              <div className="divide-y divide-gray-100">
                {unit.exercises.map((exercise) => (
                  <label
                    key={exercise}
                    className="flex items-center space-x-3 cursor-pointer group hover:bg-red-50 px-6 py-3 transition-all"
                  >
                    <input
                      type="checkbox"
                      checked={selectedExercises.includes(exercise)}
                      onChange={() => handleExerciseCheckboxChange(exercise)}
                      className="w-5 h-5 text-red-600 rounded-lg border-gray-300 focus:ring-red-500 transition-all"
                    />
        ...
        <div className="mt-12 flex flex-col items-center">
          ...
          <button
            onClick={() => {/* Handle paper generation */}}
            className={`
              px-10 py-4 rounded-xl font-semibold text-white
              transition-all duration-300 transform
              flex items-center gap-2
              ${selectedExercises.length > 0
                ? 'bg-gradient-to-r from-red-600 via-red-500 to-orange-500 hover:scale-105 hover:shadow-xl'
...
export default PakistanStudiesNinth;