...
export default function PaperPreview() {
  ...
  const [sections, setSections] = useState({
    ...
    short: [{ title: "مختصر سوالات", questions: [], choice: 0 }],
    poet_ex: [{ title: "اشعار کی تشریح", questions: [], choice: 0 }],
    proseExp: [{ title: "نثر پاروں کی تشریح", questions: [], choice: 0 }],
    summary: [{ title: "خلاصہ", questions: [], choice: 0 }],
    cen_idea: [{ title: "مرکزی خیال حصہ نظم", questions: [], choice: 0 }],
    letter: [{ title: "خط", questions: [], choice: 0 }],
    application: [{ title: "درخواست", questions: [], choice: 0 }],
    story: [{ title: "کہانی", questions: [], choice: 0 }],
    dialogue: [{ title: "مکالمہ", questions: [], choice: 0 }],
    sen_corr: [{ title: "جملوں کی درستی", questions: [], choice: 0 }],
    provrb_exp: [{ title: "ضرب الامثال", questions: [], choice: 0 }]
  });
  ...
  useEffect(() => {
    ...
    const savedExercises = localStorage.getItem("selectedUrdu9Exercises");
    if (savedExercises) {
      try {
...
const preserveLineBreaks = (text, type) => {
  ...
  let marginBottom = "0"; // default margin
  ...
  switch (type) {
    ...
    case "proseExp":
      ...
      break;
    case "summary":
      lineHeight = paperSettings.summaryLineHeight;
      break;
    case "cen_idea":
      lineHeight = paperSettings.cen_ideaLineHeight;
      break;
    case "letter":
      lineHeight = paperSettings.letterLineHeight;
      break;
    case "application":
      lineHeight = paperSettings.applicationLineHeight;
      break;
    case "story":
      lineHeight = paperSettings.storyLineHeight;
      break;
    case "dialogue":
      lineHeight = paperSettings.dialogueLineHeight;
      break;
    case "sen_corr":
      lineHeight = paperSettings.sen_corrLineHeight;
      break;
    case "provrb_exp":
      lineHeight = paperSettings.provrb_expLineHeight;
      break;
    default:
      lineHeight = paperSettings.lineHeight;
  }
...
// ... existing code ...
  ...
  switch (questionType) {
    ...
    case "summary":
      return {
        ...baseStyle,
        lineHeight: paperSettings.summaryLineHeight || paperSettings.lineHeight,
      };
    case "cen_idea":
      return {
        ...baseStyle,
        lineHeight: paperSettings.cen_ideaLineHeight || paperSettings.lineHeight,
      };
    case "letter":
      return {
        ...baseStyle,
        lineHeight: paperSettings.letterLineHeight || paperSettings.lineHeight,
      };
    case "application":
      return {
        ...baseStyle,
    ...
    case "story":
      return {
        ...baseStyle,
        lineHeight: paperSettings.storyLineHeight || paperSettings.lineHeight,
      };
    case "dialogue":
      return {
        ...baseStyle,
    ...
    case "sen_corr":
      return {
        ...baseStyle,
        lineHeight: paperSettings.sen_corrLineHeight || paperSettings.lineHeight,
      };
    case "provrb_exp":
      return {
        ...baseStyle,
        lineHeight: paperSettings.provrb_expLineHeight || paperSettings.lineHeight,
      };
    default:
  ...
  const handleAddSelection = () => {
    ...
    setSections((prevSections) => {
      ...
      Object.entries(questionsByType).forEach(([type, questions]) => {
        ...
        switch (type) {
          case "short":
            choiceValue = Math.min(paperSettings.shortChoice || 0, questions.length - 1);
            break;
          case "poet_ex":
            choiceValue = Math.min(paperSettings.poet_exChoice || 0, questions.length - 1);
            break;
          case "proseExp":
            choiceValue = Math.min(paperSettings.proseExpChoice || 0, questions.length - 1);
            break;
          case "summary":
            choiceValue = Math.min(paperSettings.summaryChoice || 0, questions.length - 1);
            break;
          case "cen_idea":
            choiceValue = Math.min(paperSettings.cen_ideaChoice || 0, questions.length - 1);
            break;
          case "letter":
            choiceValue = Math.min(paperSettings.letterChoice || 0, questions.length - 1);
            break;
          case "application":
            choiceValue = Math.min(paperSettings.applicationChoice || 0, questions.length - 1);
            break;
          case "story":
            choiceValue = Math.min(paperSettings.storyChoice || 0, questions.length - 1);
            break;
          case "dialogue":
            choiceValue = Math.min(paperSettings.dialogueChoice || 0, questions.length - 1);
            break;
          case "sen_corr":
            choiceValue = Math.min(paperSettings.sen_corrChoice || 0, questions.length - 1);
            break;
          case "provrb_exp":
            choiceValue = Math.min(paperSettings.provrb_expChoice || 0, questions.length - 1);
            break;
          default:
            choiceValue = 0;
        }
        ...
        switch (type) {
          ...
          case "short":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} مختصر سوالات کے جوابات تحریر کریں۔`
              : "مختصر سوالات";
            break;
          case "poet_ex":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} اشعار کی تشریح کریں۔`
              : "اشعار کی تشریح";
            break;
          case "proseExp":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} نثر پاروں کی تشریح کریں۔`
              : "نثر پاروں کی تشریح";
            break;
          case "summary":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} خلاصہ تحریر کریں۔`
              : "خلاصہ";
            break;
          case "cen_idea":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} مرکزی خیال تحریر کریں۔`
              : "مرکزی خیال";
            break;
          case "letter":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} خط تحریر کریں۔`
              : "خط";
            break;
          case "application":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} درخواست تحریر کریں۔`
              : "درخواست";
            break;
          case "story":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} کہانی تحریر کریں۔`
              : "کہانی";
            break;
          case "dialogue":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} مکالمہ تحریر کریں۔`
              : "مکالمہ";
            break;
          case "sen_corr":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} جملوں کی درستی کریں۔`
              : "جملوں کی درستی";
            break;
          case "provrb_exp":
            title = choiceValue > 0
              ? `درج ذیل ${questions.length} میں سے ${requiredQuestions} ضرب الامثال کی تشریح کریں۔`
              : "ضرب الامثال";
            break;
          default:
  ...
  const handlePrintClick = () => {
    ...
    const filename = `9th-Urdu-${paperSettings.section || "section"}-${
...
`);
  ...
  const getHeadingTextByType = (type, section) => {
    ...
    switch (type) {
      ...
      case "proseExp":
        ...
        break;
      case "summary":
        choiceValue = paperSettings.summaryChoice || 0;
        break;
      case "cen_idea":
        choiceValue = paperSettings.cen_ideaChoice || 0;
        break;
      case "letter":
        choiceValue = paperSettings.letterChoice || 0;
        break;
      case "application":
        choiceValue = paperSettings.applicationChoice || 0;
        break;
      case "story":
        choiceValue = paperSettings.storyChoice || 0;
        break;
      case "dialogue":
        choiceValue = paperSettings.dialogueChoice || 0;
        break;
      case "sen_corr":
        choiceValue = paperSettings.sen_corrChoice || 0;
        break;
      case "provrb_exp":
        choiceValue = paperSettings.provrb_expChoice || 0;
        break;
    }
    ...
    switch (type) {
      ...
      case "poet_ex":
        return `اشعار کی تشریح (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      case "proseExp":
        return `نثر پاروں کی تشریح (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      case "summary":
        return `خلاصہ (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      case "cen_idea":
        return `مرکزی خیال حصہ نظم (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      case "letter":
        return `خط (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      case "application":
        return `درخواست (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      case "story":
        return `کہانی (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      case "dialogue":
        return `مکالمہ (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      case "sen_corr":
        return `جملوں کی درستی (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      case "provrb_exp":
        return `ضرب الامثال (${requiredQuestions}×${marksPerQuestion}=${totalMarks})`;
      default:
        return "";
    }
  };
  ...
  const calculateMarksForQuestionType = (questions, type) => {
    ...
    switch (type) {
      case "short":
        choice = paperSettings.shortChoice || 0;
        break;
      case "poet_ex":
      ...
      case "proseExp":
        ...
        break;
      case "summary":
        choice = paperSettings.summaryChoice || 0;
        break;
      case "cen_idea":
        choice = paperSettings.cen_ideaChoice || 0;
        break;
      case "letter":
        choice = paperSettings.letterChoice || 0;
        break;
      case "application":
        choice = paperSettings.applicationChoice || 0;
        break;
      case "story":
        choice = paperSettings.storyChoice || 0;
        break;
      case "dialogue":
        choice = paperSettings.dialogueChoice || 0;
        break;
      case "sen_corr":
        choice = paperSettings.sen_corrChoice || 0;
        break;
      case "provrb_exp":
        choice = paperSettings.provrb_expChoice || 0;
        break;
    }
  ...
  const calculateTotalMarks = () => {
    ...
    const summaryMarks = calculateTypeMarks("summary", paperSettings.summaryChoice);
    const cen_ideaMarks = calculateTypeMarks("cen_idea", paperSettings.cen_ideaChoice);
    const letterMarks = calculateTypeMarks("letter", paperSettings.letterChoice);
    const applicationMarks = calculateTypeMarks("application", paperSettings.applicationChoice);
    const storyMarks = calculateTypeMarks("story", paperSettings.storyChoice);
    const dialogueMarks = calculateTypeMarks("dialogue", paperSettings.dialogueChoice);
    const sen_corrMarks = calculateTypeMarks("sen_corr", paperSettings.sen_corrChoice);
    const provrb_expMarks = calculateTypeMarks("provrb_exp", paperSettings.provrb_expChoice);

    return (
      mcqsMarks + shortMarks + poet_exMarks + proseExpMarks + summaryMarks + 
      cen_ideaMarks + letterMarks + applicationMarks + storyMarks + 
      dialogueMarks + sen_corrMarks + provrb_expMarks
    );
  };
  ...
  const calculatePaperTotalMarks = () => {
    ...
    const questionTypes = [
      "mcqs", "short", "poet_ex", "proseExp", "summary", 
      "cen_idea", "letter", "application", "story", 
      "dialogue", "sen_corr", "provrb_exp"
    ];
  ...
  <div className="grid grid-cols-3 gap-6">
    <div className="flex items-center gap-3">
      ...
      >
        ...
        <option  className="font-nastaliq" value="poet_ex">اشعار کی تشریح</option>
        <option  className="font-nastaliq" value="proseExp">نثر پاروں کی تشریح</option>
        <option  className="font-nastaliq" value="short">مختصر سوالات</option>
        <option  className="font-nastaliq" value="summary">خلاصہ</option>
        <option  className="font-nastaliq" value="cen_idea">مرکزی خیال حصہ نظم</option>
        <option  className="font-nastaliq" value="letter">خط</option>
        <option  className="font-nastaliq" value="application">درخواست</option>
        <option  className="font-nastaliq" value="story">کہانی</option>
        <option  className="font-nastaliq" value="dialogue">مکالمہ</option>
        <option  className="font-nastaliq" value="sen_corr">جملوں کی درستی</option>
        <option  className="font-nastaliq" value="provrb_exp">ضرب الامثال</option>
      </select>
    </div>
    ...
    <div className="grid grid-cols-4 gap-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <input
          type="number"
          min="0"
          value={paperSettings.summaryChoice || 0}
          onChange={(e) => setPaperSettings(prev => ({...prev, summaryChoice: Number.parseInt(e.target.value) || 0}))}
          className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
        />
      </div>
    </div>

    {/* Row 3: Choice Controls - Second Row */}
    <div className="grid grid-cols-4 gap-4 mt-2">
      <div className="flex items-center gap-2">
        ...
        <input
          type="number"
          min="0"
          value={paperSettings.cen_ideaChoice || 0}
          onChange={(e) => setPaperSettings(prev => ({...prev, cen_ideaChoice: Number.parseInt(e.target.value) || 0}))}
          className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
        />
      </div>
      <div className="flex items-center gap-2">
        ...
        <input
          type="number"
          min="0"
          value={paperSettings.letterChoice || 0}
          onChange={(e) => setPaperSettings(prev => ({...prev, letterChoice: Number.parseInt(e.target.value) || 0}))}
          className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
        />
      </div>
      <div className="flex items-center gap-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <input
          type="number"
          min="0"
          value={paperSettings.storyChoice || 0}
          onChange={(e) => setPaperSettings(prev => ({...prev, storyChoice: Number.parseInt(e.target.value) || 0}))}
          className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
        />
      </div>
    </div>

    {/* Row 4: Choice Controls - Third Row */}
    <div className="grid grid-cols-4 gap-4 mt-2">
      <div className="flex items-center gap-2">
        <div className={`bg-violet-100 text-violet-700 px-3 py-2 text-xs rounded-lg font-medium shadow-sm whitespace-nowrap font-nastaliq`}>
          مکالمہ
      ...
      <div className="flex items-center gap-2">
        ...
        <input
          type="number"
          min="0"
          value={paperSettings.sen_corrChoice || 0}
          onChange={(e) => setPaperSettings(prev => ({...prev, sen_corrChoice: Number.parseInt(e.target.value) || 0}))}
          className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
        />
      </div>
      <div className="flex items-center gap-2">
        ...
        <input
          type="number"
          min="0"
          value={paperSettings.provrb_expChoice || 0}
          onChange={(e) => setPaperSettings(prev => ({...prev, provrb_expChoice: Number.parseInt(e.target.value) || 0}))}
          className="w-full border border-violet-100 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30"
        />
      </div>
      <div className="flex items-center gap-2">
