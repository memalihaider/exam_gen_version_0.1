...
// import { ninthMathematics} from "@/data/dummyQuestions"
...
export default function PaperPreview() {
  ...
  useEffect(() => {
    ...
    const savedExercises = localStorage.getItem("selectedMathematicsExercises");
    if (savedExercises) {
      try {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const subject = paperSettings.subject || "Mathematics"; // Get current subject
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const params = new URLSearchParams({
          class: "9th",
          subject: "Mathematics", // Always use Mathematics for the initial fetch
        });
  ...
  const handleSearchQuestions = () => {
    ...
    // Build query parameters - IMPORTANT: Always use Mathematics as the subject since that's what the topics are for
    const params = new URLSearchParams({
      class: paperSettings.class,
      subject: "Mathematics", // Hardcode to Mathematics since the topics are from Mathematics
    });
  ...
  const handlePrintClick = () => {
    ...
    const filename = `9th-Mathematics-${paperSettings.section || "section"}-${
...
`);
  ...
  const handleSavePaper = async () => {
    try {
      ...
      const paperData = {
        ...
        subject: paperSettings.subject || "Mathematics",
  ...
  const determineOptionLayout = (options, hasParts) => {
    ...
    // Check if any option is longer than a certain threshold or contains math
    const hasLongOptions = options.some((option) => option.length > 25);
    const hasVeryLongOptions = options.some((option) => option.length > 50);
    const hasMathOptions = options.some((option) => option.includes('$'));
  
    // Add extra margin if question has parts
    const partsMargin = hasParts ? "mt-1" : "mt-0";
  
    // If any option contains math or is very long, use vertical layout
    if (hasVeryLongOptions || hasMathOptions) {
      // Stack layout (1 per row) for very long options or math options
      return (
  ...
  const renderMathText = (text) => {
    ...
    try {
      ...
      if (blockMatches.length === 0) {
        // No block math, just handle inline math
        return renderInlineMath(text);
      }
      
      blockMatches.forEach((blockMatch, index) => {
        // Add text before math (with inline math processing)
        if (blockMatch.index > currentIndex) {
          const beforeText = text.slice(currentIndex, blockMatch.index);
          const inlineParts = renderInlineMath(beforeText);
          parts.push(...(Array.isArray(inlineParts) ? inlineParts : [inlineParts]));
        }
        
        // Add block math
        parts.push(
          <div key={`block-${blockMatch.index}`} className="my-2 text-center">
      ...
      if (currentIndex < text.length) {
        ...
        const inlineParts = renderInlineMath(remainingText);
        parts.push(...(Array.isArray(inlineParts) ? inlineParts : [inlineParts]));
      }
  ...
  const renderInlineMath = (text) => {
    ...
    try {
      ...
      // Handle inline math ($...$) but not block math ($$...$$)
      const inlineMathRegex = /(?<!\$)\$(?!\$)([^$]+?)\$(?!\$)/g;
      const inlineMatches = [...text.matchAll(inlineMathRegex)];
      
      if (inlineMatches.length === 0) {
        return text;
      }
      
      inlineMatches.forEach((inlineMatch, index) => {
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td className="border border-gray-200 p-2 w-1/4 bg-gray-50">
                            ...
                            <span className="text-gray-700 text-sm">
                              {paperSettings.subject || "Mathematics"}
                            </span>
                          </td>
